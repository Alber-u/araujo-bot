// Build: 2026-06-27 v18.190 (Sobre v18.189: el boton "IMPRIMIR DOCUMENTOS" (que ya existia en la ficha, en la cabecera de la caja Datos CCPP) se anade TAMBIEN al menu hamburguesa, junto a "PRESUPUESTO PLAN 5". Nuevo _imprimirDocsItem en renderCabeceraComun que solo aparece cuando hay expediente abierto (opts.expedienteId) y, al pulsarlo, cierra el menu y dispara el clic del boton existente ptlBtnImprimirDocs -reutiliza el mismo modal y endpoints /docs/menu,/docs/huecos,/docs/generar; no duplica logica-. Si el boton no estuviera en la pagina, avisa en vez de fallar. Sin cambios en la ficha ni en el flujo de generacion del PDF. node --check OK, CRLF.)
// Build: 2026-06-27 v18.189 (Sobre v18.188: ajuste visual de la caja Datos economicos en las fases NO tramitadas. Los 5 campos previstos (PTO total, Tiempo previsto, Mano de obra previsto, Material previsto, Beneficio previsto) vuelven a quedar APILADOS uno debajo de otro en la columna izquierda (cada uno col-4 + relleno col-8), como estaban antes, en vez de tiempo/mano de obra/material en una sola fila de tres. Solo maquetacion del grid de solo-previstos; el de 09_TRAMITADA y todo lo demas intacto. node --check OK, CRLF.)
