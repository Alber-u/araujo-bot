        ...
        return `
          <div class="hoy-exp-bloque" data-ccpp-id="${_esc(c.ccpp_id)}">
            <div class="hoy-exp-fila" data-ccpp-id="${_esc(c.ccpp_id)}" style="display:grid;grid-template-columns:repeat(8,1fr);align-items:center;gap:6px;padding:0 6px;border-bottom:1px solid var(--ptl-gray-100);min-height:22px;font-size:11px;line-height:1.1;background:${bgCab}">
              ...
              <span style="grid-column:5;display:flex;align-items:center;justify-content:flex-end;white-space:nowrap;min-width:0">${faseC === "05_DOCUMENTACION" ? _badgeUltimatumHoy(c, _contactoBotPorCcpp[String(c.comunidad || c.direccion || "").trim().toLowerCase()] || "", _plazosUlt) : faseC === "08_CYCP" ? _badgeUltimatumHoy(c, String(c.fecha_envio_contratos_pagos || "").slice(0, 10), _plazosUltCycp, _CFG_ULT8) : ""}</span>
              <span style="grid-column:6;display:flex;align-items:center;justify-content:flex-end;white-space:nowrap;min-width:0">${badgeHoy || ""}</span>
              <span style="grid-column:7;display:flex;align-items:center;justify-content:flex-end;white-space:nowrap;min-width:0">${pillFaltanHoy || ""}</span>
              ${conReloj
                ? `<button type="button" class="ptl-vec-btn hoy-exp-reloj ptl-btn-reloj" data-ccpp-id="${_esc(c.ccpp_id)}" data-pisos-activos="${pisos.length}" data-enhoy="1" title="Quitar de HOY" style="grid-column:8;justify-self:end;width:18px;height:18px;font-size:9px">⏰</button>`
                : `<span style="grid-column:8"></span>`}
            </div>
            ${filasPisos}
          </div>
        `;
