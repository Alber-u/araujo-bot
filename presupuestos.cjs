    ...
    try {
      ...
      const renderExpedienteEnHoy = (c, bloqueIdx, conReloj = true) => {
        ...
        if (_f) {
          ...
          pillFaltanHoy = `<span class="ptl-fila-badge ${_cls}" style="flex:0 0 96px;text-align:center">${_esc(_f.texto)}</span>`;
        }
        return `
      ...
      try {
        ...
        function _renderFilaExp(c, plantilla, faltan, infoEnvio, idx, estadoPlazo) {
          ...
          if (faltan.totalFilas === 0) {
            pillFaltan = `<span class="ptl-fila-badge ptl-fila-badge-neutro">sin pisos</span>`;
          } else if (pendientes === 0) {
            pillFaltan = `<span class="ptl-fila-badge ptl-fila-badge-success">✓ Completo</span>`;
          } else {
            pillFaltan = `<span class="ptl-fila-badge ptl-fila-badge-danger">Faltan ${pendientes} de ${faltan.totalFilas}</span>`;
          }
