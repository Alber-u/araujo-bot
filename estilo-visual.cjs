// estilo-visual.cjs
// Build: 2026-06-27 v1.97 (Sobre v1.96: el texto del boton "Enviar presupuesto y paso a 04" (.ptl-btn-enviar-avanzar) quedaba pegado ARRIBA dentro del boton alto de la cinta 2col, en vez de centrado. CAUSA: .ptl-na-igual-altura .ptl-btn (especificidad 2 clases) pisaba el flex-direction:column/justify-content:center que el boton ya traia (1 clase). FIX: se amplia la regla .ptl-next-action-grid-2col .ptl-na-right .ptl-btn-enviar-avanzar (3 clases, gana) para imponer display:flex;flex-direction:column;justify-content:center;align-items:center -> las dos lineas (.ln) se apilan y quedan centradas vertical y horizontalmente en el boton. Solo afecta a ese boton en la cinta 2col de la fase 03. node --check OK, CRLF.)
