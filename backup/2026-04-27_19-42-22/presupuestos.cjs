...
<html lang="es"><head>
  ...
  // =================================================================
  function vistaNuevo(error, token, tiposVia) {
    const opciones = (tiposVia || []).map(t => `<option value="${esc(t)}">`).join("");
    return `
      ...
      <form method="POST" action="${urlT(token, "/presupuestos/nuevo")}">
        <div class="ptl-card">
          ...
          <div class="ptl-form-grid">
            <div class="col-9"><label class="ptl-form-label">Dirección *</label><input name="direccion" required autofocus placeholder="Ej. Doctor Fedriani 39"/></div>
            <div class="col-3"><label class="ptl-form-label">Tipo de vía</label>
              <input name="tipo_via" list="ptl-dl-tipos-via" placeholder="(C), (Av)..." value="(C)" autocomplete="off"/>
              <datalist id="ptl-dl-tipos-via">${opciones}</datalist>
            </div>
            <div class="col-12" style="display:flex;align-items:center;gap:8px">
              <label class="ptl-form-label" style="margin:0">Earth:</label>
              <select name="earth" style="width:auto;flex:0 0 auto"><option value="NO">No</option><option value="SI">Sí</option></select>
            </div>
          </div>
          <div class="ptl-form-section-title">Administrador</div>
          <div class="ptl-form-grid">
            <div class="col-6"><label class="ptl-form-label">Nombre</label><input name="administrador"/></div>
          ...
          <div class="ptl-form-section-title">Presidente</div>
          <div class="ptl-form-grid">
            <div class="col-6"><label class="ptl-form-label">Nombre</label><input name="presidente"/></div>
  ...
  app.get("/presupuestos/nuevo", async (req, res) => {
    if (!checkToken(req, res)) return;
    const token = req.query.token || "";
    let tiposVia = ["(C)", "(Av)", "(Pl)", "(Cm)", "(Pje)", "(Bda)", "(Crta)"];
    try {
      const comunidades = await leerComunidades();
      const ts = comunidades.map(c => c.tipo_via).filter(Boolean);
      const unicos = Array.from(new Set([...tiposVia, ...ts])).filter(Boolean);
      tiposVia = unicos;
    } catch (e) {
      console.warn("[presupuestos] no se pudieron leer tipos_via:", e.message);
    }
    sendHtml(res, pageHtml("Nuevo expediente",
      [{ label: "Presupuestos", url: urlT(token, "/presupuestos") }, { label: "Nuevo", url: "#" }],
      vistaNuevo(req.query.error || "", token, tiposVia),
      token));
  });
