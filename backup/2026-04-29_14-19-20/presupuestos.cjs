...
module.exports = function (app) {
  ...
  const PTO_FASES = {
    "01_SOLICITUD":     { codigo: "01", nombre: "Solicitud",   nombreLargo: "SOLICITUD",       color: "azul",     siguiente: "02_VISITA",       accionLabel: "Solicitud registrada", plantilla: "primer_contacto", cadenciaDias: 30 },
    "02_VISITA":        { codigo: "02", nombre: "Pte. visita", nombreLargo: "PTE VISITA",      color: "azul",     siguiente: "03_ENVIO",        accionLabel: "Programar visita",     plantilla: null },
    ...
    "04_SEGUIMIENTO":   { codigo: "04", nombre: "Seguim.",     nombreLargo: "SEGUIMIENTO PTO", color: "amarillo", siguiente: "05_RESOLUCION",   accionLabel: "Seguimiento",          plantilla: "seguimiento", cadenciaDias: 15, cadenciaInicialDias: 3 },
    "05_RESOLUCION":    { codigo: "05", nombre: "Resol.",      nombreLargo: "RESOLUCION PTO",  color: "naranja",  siguiente: null,              accionLabel: "Decisión cliente",     plantilla: null },
    "ENTREGADO":        { codigo: "06", nombre: "Entregado",   nombreLargo: "ENVIO DOC",       color: "verde",    siguiente: null,              accionLabel: "Entregado",            plantilla: null },
  ...
  const COLS = [
    ...
    "mails_enviados",        // JSON: { "01_SOLICITUD": 3, "03_ENVIO": 1, ... }
    "mails_ultimo_envio",    // JSON: { "01_SOLICITUD": "2026-04-27", ... }
  ];
  ...
  function calcularLineaTiempo(comu) {
    ...
    return [
      { proceso: "Presupuesto", nombre: "01-Solicit.", faseId: "01_SOLICITUD",   estado: estadoHito("01_SOLICITUD",   fase, idx) },
      ...
      { proceso: "Presupuesto", nombre: "05-Resol.",   faseId: "05_RESOLUCION",  estado: estadoHito("05_RESOLUCION",  fase, idx) },
...
<html lang="es"><head>
  ...
  const CSS = `
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .ptl-filtros-fases .ptl-filtro{flex-shrink:0;padding:4px 7px;font-size:10px}
    ...
    /* Filas de lista */
    .ptl-fila{background:white;border:1px solid var(--ptl-gray-200);border-radius:8px;padding:3px 12px;margin-bottom:3px;display:flex;align-items:center;gap:8px;color:inherit;transition:all .15s}
    ...
    /* Timeline */
    .ptl-timeline{display:flex;align-items:stretch;gap:0;padding:2px 0 1px;overflow:hidden}
    .ptl-grupo{flex:0 0 auto;display:flex;flex-direction:column;padding:0 4px}
    ...
    .ptl-puntos{display:flex;gap:0;padding:0 2px}
    ...
    .ptl-fila .ptl-grupo{padding:0 2px}
    .ptl-fila .ptl-grupo-titulo{display:none}
    .ptl-fila .ptl-puntos{padding:0}
    .ptl-fila .ptl-punto{min-width:34px}
  ...
  function vistaListado(comunidades, query, token) {
    ...
    ["01_SOLICITUD","02_VISITA","03_ENVIO","04_SEGUIMIENTO","05_RESOLUCION","ENTREGADO","ZZ_RECHAZADO","ZZ_DESCARTADO"].forEach(f => counts[f] = 0);
    const FASES_ACTIVAS = ["01_SOLICITUD","02_VISITA","03_ENVIO","04_SEGUIMIENTO","05_RESOLUCION","ENTREGADO"];
    const FASES_EN_TRAMITE = ["04_SEGUIMIENTO","05_RESOLUCION","ENTREGADO"];
    ...
    const sumaProcesos = counts["01_SOLICITUD"]+counts["02_VISITA"]+counts["03_ENVIO"]+counts["04_SEGUIMIENTO"]+counts["05_RESOLUCION"]+counts["ENTREGADO"]+counts["ZZ_RECHAZADO"]+counts["ZZ_DESCARTADO"];
    ...
    return `
      <div class="ptl-lista-header">
        ...
        <div class="ptl-filtros ptl-filtros-fases">
          ${filtroBtn("01_SOLICITUD", "01-SOLICITUD")}
          ${filtroBtn("02_VISITA", "02-PTE VISITA")}
          ${filtroBtn("03_ENVIO", "03-ENVIO")}
          ${filtroBtn("04_SEGUIMIENTO", "04-SEGUIM.")}
          ${filtroBtn("05_RESOLUCION", "05-RESOL.")}
          ${filtroBtn("ENTREGADO", "06-ENVIO DOC")}
          ${filtroBtn("ZZ_RECHAZADO", "ZZ-RECHAZ.")}
          ${filtroBtn("ZZ_DESCARTADO", "ZZ-DESCART.")}
        </div>
      </div>
      <div>
  ...
  function vistaFicha(comu, vecinos, datalists, token, todosExpedientes, reciencreado) {
    ...
    if (fase === "ZZ_RECHAZADO") {
      ...
      accionHtml = `<div class="ptl-next-action" style="background:var(--ptl-gray-50);border-color:var(--ptl-gray-200)">
        ...
        <form method="POST" action="${urlT(token, "/presupuestos/expediente/reactivar")}" style="display:inline">
          ...
          <button type="submit" class="ptl-btn ptl-btn-primary ptl-btn-sm" onclick="return confirm('¿Reactivar el expediente? Volverá a 01-SOLICITUD con los contadores reseteados.')">↻ Reactivar expediente</button>
        </form>
      </div>`;
      ...
      // Fases activas con email asociado: 01_SOLICITUD, 03_ENVIO
      ...
      // Texto indicador con código + nombre + fecha de la fase
      const fechaActual = fechaHito(comu, fase);
      const fechaTxt = fechaActual ? ` · ${fmtFecha(fechaActual)}` : '';
      let labelFaseActual = `${def.codigo}-${(def.nombreLargo || def.nombre || '').toUpperCase()}${fechaTxt}`;
      ...
      // Botón mail: depende de si hay 0 envíos o ya hay
      let btnMailHtml = '';
      if (tienePlantilla) {
        if (numEnviosFase === 0) {
          // Sin envíos: botón "Activar envíos automáticos" (manda 1er + activa cron)
          btnMailHtml = `<button type="button" class="ptl-btn ptl-btn-secondary ptl-btn-sm"
            onclick="ptlAbrirModalMail('${esc(fase)}', '${esc(comu.ccpp_id)}')">📧 Activar envíos automáticos</button>`;
        }
        // Si ya hay envíos, NO mostramos el botón (el cron lo gestiona)
      }

      accionHtml = `<div class="ptl-next-action">
        <div class="ico">→</div>
        <div style="flex:1"><div class="text">${esc(labelFaseActual)}</div></div>
        ${btnMailHtml}
    ...
    const inp = (name, val, opts = {}) => {
      ...
      else if (esEuros) cls = ' class="campo-euros" inputmode="decimal"';
      else if (esDias)  cls = ' class="campo-dias" inputmode="decimal"';
    ...
    return `
      ...
      <form id="ptl-ficha-form" data-id="${esc(comu.ccpp_id)}" onsubmit="return false">
        ...
        <div class="ptl-card">
          ...
          <div class="ptl-form-grid">
            ${inp("pto_total", comu.pto_total, { type: "number", formato: "euros", col: 12, label: "PTO total (€)" })}
            ${inp("tiempo_previsto", comu.tiempo_previsto, { type: "number", formato: "dias", col: 4, label: "Tiempo previsto (días/cuadrilla × 2)" })}
            ${inp("tiempo_real",     comu.tiempo_real,     { type: "number", formato: "dias", col: 4, label: "Tiempo real (días/cuadrilla × 2)" })}
            <div class="col-4">
            ...
            ${inp("mano_obra_previsto", comu.mano_obra_previsto, { type: "number", formato: "euros", col: 6, label: "Mano de obra previsto" })}
            ${inp("mano_obra_real",     comu.mano_obra_real,     { type: "number", formato: "euros", col: 6, label: "Mano de obra real" })}
            ${inp("material_previsto",  comu.material_previsto,  { type: "number", formato: "euros", col: 6, label: "Material previsto" })}
            ${inp("material_real",      comu.material_real,      { type: "number", formato: "euros", col: 6, label: "Material real" })}
            <div class="col-4">
      ...
      <div class="ptl-save-bar">
        <form method="POST" action="${urlT(token, "/presupuestos/expediente/descartar")}" style="display:inline">
          <input type="hidden" name="id" value="${esc(comu.ccpp_id)}"/>
          <button type="submit" class="ptl-btn ptl-btn-danger" onclick="if (confirm('¿Enviar este expediente a descartados?\\n\\nPasará a ZZ-DESCARTADO y no podrá enviarse más.\\nPodrás reactivarlo desde el listado de descartados.')) { window.ptlIntercept = false; window.ptlEliminando = true; return true; } return false;">📦 Enviar a descartados</button>
        </form>
      </div>
          ...
          try {
            ...
            if (max > 0) {
              ...
              if (enviados + 1 >= max) {
                ...
                aviso.innerHTML = '⚠ Este será el último envío permitido. Después el expediente pasará a <strong>ZZ-DESCARTADO</strong>.';
              }
            } else {
            ...
            btn.onclick = async () => {
              ...
              try {
                ...
                msg += '\\n\\nEl sistema gestionará los siguientes envíos automáticamente.';
                alert(msg);
                ptlCerrarModalMail();
  ...
  // POST /presupuestos/expediente/reactivar — vuelve a 01_SOLICITUD reseteando contadores
  ...
  app.post("/presupuestos/expediente/reactivar", async (req, res) => {
    ...
    try {
      ...
      comu.fase_presupuesto = "01_SOLICITUD";
  ...
  // GET /presupuestos/plantilla-mail?fase=01_SOLICITUD&id=...
  ...
  // =================================================================
  // CRON INTERNO: revisa fichas en 01_SOLICITUD para enviar mails automáticos
  // =================================================================
  // Filosofía:
  //  - Solo actúa sobre fichas con mails_enviados.01_SOLICITUD >= 1 (cron activado)
  ...
  //  - Para fase 01_SOLICITUD (extensible al resto en el futuro)
  const CRON_FASES_AUTO = ["01_SOLICITUD"];
  ...
  app.get("/presupuestos/plantillas", async (req, res) => {
    ...
    try {
      ...
      const fasesConPlantilla = ["01_SOLICITUD", "03_ENVIO", "04_SEGUIMIENTO"];
      const plantillas = [];
