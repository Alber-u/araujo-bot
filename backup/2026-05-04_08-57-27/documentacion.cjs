...
module.exports = function (app) {
  ...
  function filaManualHtml(opciones) {
    ...
    // En las filas de piso, los 3 botones: + guardar, 📄 acordeón, ✕ borrar.
  ...
  function cajitaManualHtml({ comu, pisos, expedientes, docsManuales, estadosCcpp, esc, fmtTlf, token }) {
    ...
    // En esta fase, ccpp_contrato y ccpp_pago no aplican (corresponden a fase
    // 07_CONTRATOS_PAGOS). Los excluimos de la cajita pero conservamos sus
    // estados en el Sheet por si en otra fase aparecen.
    const CCPP_OCULTOS = new Set(["ccpp_contrato", "ccpp_pago"]);
    const docsCcpp = [];
    const estadosCcppFiltrados = [];
    for (let i = 0; i < docsCcppCompletos.length; i++) {
      const d = docsCcppCompletos[i];
      if (CCPP_OCULTOS.has(d.codigo)) continue;
      docsCcpp.push(d);
    ...
    if (docsPiso.length === 0 && docsCcpp.length === 0) {
      return `<div class="ptl-card" style="margin-top:12px">
        <div class="ptl-card-title">DATOS PISOS</div>
    ...
    <div class="ptl-card ptl-vec-card-manual"
      ...
      <div class="ptl-card-title-row" style="display:flex; align-items:center; gap:8px;">
        <span class="ptl-card-title">DATOS PISOS</span>
  ...
  app.get("/documentacion/expediente", async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        cajitaManual = `<div class="ptl-card" style="margin-top:12px"><b>DATOS PISOS</b><br><small style="color:#666">No se pudo cargar: ${P.esc(e.message)}</small></div>`;
      }
