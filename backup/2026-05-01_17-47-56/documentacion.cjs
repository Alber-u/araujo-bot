...
module.exports = function (app) {
  ...
  function filaPisoHtml(piso, expByTlf, esc, fmtTlf) {
    ...
    // Contar docs "hechos" = no pendientes. Sumamos recibidos + sin archivo + no aplica.
    const TOTAL_DOCS = DOCS_UNIVERSAL.length;
    let docsHechos = 0, docsHtml = `<span class="ptl-vec-docs-tag ptl-vec-docs-rojo">—</span>`;
    if (exp) {
      const recibidos = (exp.documentos_recibidos || "").split(",").filter(Boolean);
      const sinArch   = (exp.documentos_recibidos_sin_archivo || "").split(",").filter(Boolean);
      const noAplica  = (exp.documentos_no_aplica || "").split(",").filter(Boolean);
      const set = new Set([...recibidos, ...sinArch, ...noAplica].filter(c => DOCS_UNIVERSAL.includes(c)));
      docsHechos = set.size;
      const cls = docsHechos >= TOTAL_DOCS ? "ptl-vec-docs-verde" : "ptl-vec-docs-rojo";
      docsHtml = `<span class="ptl-vec-docs-tag ${cls}">${docsHechos}/${TOTAL_DOCS}</span>`;
    } else if (!tlf) {
      docsHtml = `<span class="ptl-vec-docs-tag ptl-vec-docs-gris">—</span>`;
    } else {
      // Tiene teléfono pero no expediente: 0/23 en rojo
      docsHtml = `<span class="ptl-vec-docs-tag ptl-vec-docs-rojo">0/${TOTAL_DOCS}</span>`;
    }
  ...
  function cajitaVecinosHtml(comu, pisos, expedientes, token, esc, fmtTlf) {
    ...
    // Un piso se considera "completo" cuando ninguno de los 23 documentos
    // universales está en estado "pendiente" (azul). Es decir, todos están
    // en alguno de: recibido_archivo / recibido_sin_archivo / no_aplica.
    const TOTAL_DOCS = DOCS_UNIVERSAL.length; // 23
    function contarDocsPiso(exp) {
      if (!exp) return { hechos: 0, total: TOTAL_DOCS };
      const recibidos = (exp.documentos_recibidos || "").split(",").filter(Boolean);
      const sinArch   = (exp.documentos_recibidos_sin_archivo || "").split(",").filter(Boolean);
      const noAplica  = (exp.documentos_no_aplica || "").split(",").filter(Boolean);
      // Set para no contar dobles
      const hechosSet = new Set([...recibidos, ...sinArch, ...noAplica]);
      // Solo cuentan los que están en la lista universal
      let hechos = 0;
      for (const d of DOCS_UNIVERSAL) if (hechosSet.has(d)) hechos++;
      return { hechos, total: TOTAL_DOCS };
    }
    const total = pisos.length;
    let pisosCompletos = 0;
    for (const p of pisos) {
      const exp = expByTlf[normTlfKey(p.telefono)];
      const { hechos, total: tt } = contarDocsPiso(exp);
      if (hechos >= tt) pisosCompletos++;
    }
    const ccppCompleto = total > 0 && pisosCompletos === total;

    // ----- Cabecera dinámica: "Y de X completos" ó "✓ Completo" -----
    let cabeceraTextoHtml, cabeceraClase;
    if (total === 0) {
      cabeceraTextoHtml = `VECINOS · DOCUMENTACIÓN <span class="ptl-vec-total">(0)</span>`;
      cabeceraClase = "ptl-vec-cab-gris";
    } else if (ccppCompleto) {
      cabeceraTextoHtml = `VECINOS · DOCUMENTACIÓN <span class="ptl-vec-cab-tag">✓ Completo</span>`;
      cabeceraClase = "ptl-vec-cab-verde";
    } else {
      cabeceraTextoHtml = `VECINOS · DOCUMENTACIÓN <span class="ptl-vec-cab-tag">${pisosCompletos} de ${total} completos</span>`;
      cabeceraClase = "ptl-vec-cab-rojo";
    }
    ...
    return `
      <div class="ptl-card ptl-vec-card" data-direccion="${esc(comu.direccion)}" data-comunidad="${esc(comu.comunidad)}">
        <div class="ptl-vec-cabecera-banner ${cabeceraClase}">
          <span class="ptl-vec-cabecera-titulo">${cabeceraTextoHtml}</span>
        </div>
