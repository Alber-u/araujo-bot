...
<html lang="es"><head>
  ...
  const CSS = `
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .ptl-btn-enviar-avanzar{font-size:13px;font-weight:700;padding:8px 16px;white-space:normal;line-height:1.2}
    ...
    if (fase === "ZZ_RECHAZADO") {
      ...
      if (fase === "03_ENVIO") {
        accionHtml = `<div class="ptl-next-action ptl-next-action-grid ptl-next-action-grid-2col">
          ...
          <button type="button" class="ptl-btn ptl-btn-primary ptl-btn-enviar-avanzar"
            onclick="ptlAbrirModalMail('${esc(fase)}', '${esc(comu.ccpp_id)}')"
            title="Abre el modal para revisar y enviar el presupuesto. Al confirmar, también pasa a fase 04-SEGUIMIENTO PTO.">
            📧 Enviar presupuesto y Paso a 04-SEGUIMIENTO PTO
          </button>
        </div>`;
      } else {
