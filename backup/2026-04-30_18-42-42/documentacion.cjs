...
module.exports = function (app) {
  ...
  async function leerVecinosBase() {
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      out.push({
        ...
        _rowIndex: i + 1, // fila real en Sheet (1-based, igual que Sheets)
      });
    }
    return out;
  }
  ...
  function cajitaVecinosHtml(comu, vecinosBase, expedientes, token, esc) {
    ...
    vBase.forEach(vb => {
      ...
      filas.push({
        _rowIndex: vb._rowIndex,        // identificador único: nº de fila en vecinos_base
    ...
    vExp.forEach(e => {
      ...
      if (!vBase.some(vb => normTlf(vb.telefono) === k)) {
        filas.push({
          _rowIndex: null,                  // huérfano: no hay fila en vecinos_base
          telefono: e.telefono,
          nombre: e.nombre || "",
          vivienda: e.vivienda || "",
    ...
    const filasHtml = filas.map((f, idx) => {
      ...
      return `<tr class="ptl-vecino-row" data-tlf="${esc(f.telefono)}">
        ...
        <td class="ptl-col-acciones">
          <button type="button" class="ptl-btn-circle ptl-btn-circle-blue" disabled title="Vecino ya creado">+</button>
          ...
          <button type="button" class="ptl-btn-circle ptl-btn-circle-red" title="Borrar vecino" onclick="ptlDoc.borrarVecino(${f._rowIndex || 'null'}, '${esc(f.telefono)}', '${esc(f.nombre || f.vivienda || f.telefono || '(sin datos)')}')">×</button>
        </td>
      </tr>
      ${acordeon}`;
    }).join("");
    ...
    const filaNueva = `
      <tr class="ptl-vecino-nuevo">
        ...
        <td class="ptl-col-acciones">
          ...
          <button type="button" class="ptl-btn-circle ptl-btn-circle-gray" disabled title="Disponible al añadir">📄</button>
          <button type="button" class="ptl-btn-circle ptl-btn-circle-red" disabled title="Disponible al añadir">×</button>
        </td>
      </tr>`;
    ...
    const cssInline = `
      <style>
        ...
        .ptl-col-vivienda{width:80px}
        .ptl-col-nombre{width:auto}
        .ptl-col-tel{width:130px}
        .ptl-col-estado{width:105px}
        .ptl-col-docs{width:70px}
        .ptl-col-acciones{width:110px;text-align:right;white-space:nowrap}
        ...
        .ptl-btn-circle:disabled{opacity:.35;cursor:not-allowed;transform:none;box-shadow:none}
        .ptl-btn-circle:disabled:hover{transform:none;box-shadow:none}
        .ptl-btn-circle-blue{background:var(--ptl-brand);color:#fff;border-color:var(--ptl-brand);font-weight:bold;font-size:18px}
        .ptl-btn-circle-blue:hover{background:#4338CA}
        .ptl-btn-circle-red{background:var(--ptl-danger);color:#fff;border-color:var(--ptl-danger);font-weight:bold;font-size:16px}
        .ptl-btn-circle-red:hover{filter:brightness(.92)}
  ...
  function jsClienteVecinos(token, ccppId) {
    ...
    return `
      ...
      (function () {
        ...
        ptlDoc.borrarVecino = async function (rowIndex, telefono, etiqueta) {
          const msg = "¿Seguro que quieres borrar este vecino y TODOS sus documentos?\\n\\n" +
            "Vecino: " + (etiqueta || telefono || "(sin datos)") + "\\n\\n" +
          ...
          try {
            const r = await fetch(urlT("/documentacion/vecino/borrar"), {
              method: "POST", headers: { "Content-Type": "application/json" },
              body: JSON.stringify({ rowIndex, telefono }),
            });
            if (!r.ok) throw new Error(await r.text());
            location.reload();
  ...
  // Body: { rowIndex, telefono }
  // - rowIndex: fila en vecinos_base (clave única, también para vecinos sin teléfono).
  //   Puede ser null si el vecino es huérfano (solo expediente, sin vecinos_base).
  // - telefono: opcional, solo para identificar expediente y documentos.
  // Borra: vecinos_base (si rowIndex) + expedientes + documentos + archivos en Drive.
  app.post("/documentacion/vecino/borrar", express.json(), async (req, res) => {
    if (!checkAuth(req, res)) return;
    try {
      const { rowIndex, telefono } = req.body || {};
      // Necesitamos al menos uno de los dos para identificar al vecino.
      if (!rowIndex && !telefono) {
        return res.status(400).json({ error: "Falta identificador (rowIndex o telefono)" });
      }

      const resumen = { vecinos_base: 0, expedientes: 0, documentos: 0, archivos_drive: 0, errores: [] };

      // Si tenemos rowIndex pero no teléfono, miramos qué tiene esa fila
      // (por si tiene teléfono asociado para limpiar también su expediente).
      let tlfReal = telefono;
      if (rowIndex && !tlfReal) {
        try {
          const todos = await leerVecinosBase();
          const v = todos.find(x => x._rowIndex === Number(rowIndex));
          if (v) tlfReal = v.telefono;
        } catch (e) { /* seguimos */ }
      }

      // 1) Documentos del vecino + sus archivos en Drive (solo si hay teléfono)
      if (tlfReal && normalizarTelefono(tlfReal)) {
        try {
          const docs = await leerDocumentosDeVecino(tlfReal);
        ...
        // 2) Expediente
        try {
          const exp = await buscarExpedientePorTelefono(tlfReal);
      ...
      // 3) Fila en vecinos_base (por número de fila exacta)
      if (rowIndex) {
        try {
          await borrarFilaSheet("vecinos_base", Number(rowIndex));
