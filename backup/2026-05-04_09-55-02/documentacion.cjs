...
module.exports = function (app) {
  ...
  function filaManualHtml(opciones) {
    ...
    // En la fila CCPP solo dejamos el botón de acordeón (📄).
    // En las filas de piso, los 3 botones en orden: + guardar, ✕ borrar, 📄 acordeón.
    ...
    <tr class="ptl-vec-acordeon-fila" style="display:none">
      <td colspan="5" class="ptl-vec-acordeon-cont"></td>
    </tr>`;
  }
  ...
  function cajitaManualHtml({ comu, pisos, expedientes, docsManuales, estadosCcpp, esc, fmtTlf, token }) {
    ...
    // En esta fase (05_DOCUMENTACION), los documentos *_contrato y *_pago no
    // aplican (corresponden a fase 07_CONTRATOS_PAGOS): ni los del CCPP ni los
    // del piso. Los excluimos de la cajita pero conservamos sus estados en el
    // Sheet por si en otra fase aparecen.
    const OCULTOS_FASE_05 = new Set([
    ...
    for (let i = 0; i < docsCcppCompletos.length; i++) {
      ...
      if (OCULTOS_FASE_05.has(d.codigo)) continue;
      docsCcpp.push(d);
      estadosCcppFiltrados.push(estadosCcpp[i] || "");
    }
    // Filtro paralelo para los docs del piso. Guardamos los índices de los
    // docs visibles (sobre la lista completa) para poder filtrar los estados
    // de cada piso al construir las filas.
    const docsPiso = [];
    ...
    const dataPisos = pisos.map(p => {
      ...
      return { id: "piso-" + (p.vivienda || ""), vivienda: p.vivienda || "", estados };
    });
    ...
    <div class="ptl-card ptl-vec-card-manual"
      ...
      <script>
        (function() {
          ...
          function renderAcordeon(cont, docs, estados) {
            const items = docs.map((d, i) => {
              const e = (estados[i] || '').trim();
              const txt   = textoBoton(e);
              const color = colorBoton(e);
              return '<div class="ptl-vec-doc-fila">' +
                '<button type="button" class="ptl-vec-doc-btn-manual ptl-vec-doc-btn-' + color + '"' +
          ...
          function abrirMenu(btn) {
            ...
            const opciones = permiteFin ? ESTADOS_PAGO : ESTADOS_BASICOS;
          ...
          function actualizarBoton(btn, nuevoEstado) {
            ...
            if (id === 'ccpp') {
              ...
              const dp = dataPisos.find(p => p.id === id);
              if (dp) dp.estados[idx] = nuevoEstado;
            }
            // Recalcular el contador "Docs" de la fila
            recalcularDocs(filaPiso);
          }
          ...
          tbody.addEventListener('click', e => {
            ...
            renderAcordeon(cont, docs, estados);
            acord.style.display = '';
