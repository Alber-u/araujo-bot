...
module.exports = function (app) {
  ...
  const MAIL_PLANTILLAS_DEFAULT = {
    "01_SOLICITUD": {
      activo: "SI",
      asunto: "Solicitud de aprobación de presupuesto - {{direccion}}",
      mensaje: "Buenos días,\n\nSolicitamos la aprobación en Acta de Asamblea del presupuesto para los trabajos de individualización de contadores en {{direccion}}.\n\nUn saludo,\nInstalaciones Araujo",
      adjuntos_fijos: "",
      dias_recurrente: 30,
      max_envios: 3,
    },
    "03_ENVIO": {
      activo: "SI",
      asunto: "Presupuesto - {{direccion}}",
      mensaje: "Buenos días,\n\nAdjunto presupuesto para individualización de contadores en {{direccion}}.\n\nA la espera de noticias.\n\nInstalaciones Araujo",
      adjuntos_fijos: "",
      dias_recurrente: 0,
      max_envios: 1,
    },
    "04_SEGUIMIENTO": {
      activo: "SI",
      asunto: "Seguimiento presupuesto - {{direccion}}",
      mensaje: "Buenos días,\n\nNos ponemos en contacto para hacer seguimiento del presupuesto enviado para {{direccion}}.\n\n¿Tenéis alguna duda al respecto?\n\nUn saludo,\nInstalaciones Araujo",
      adjuntos_fijos: "",
      dias_recurrente: 15,
      max_envios: 5,
    },
...
<html lang="es"><head>
  ...
  const CSS = `
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .ptl-filtros-fases{flex-wrap:nowrap;gap:4px;overflow-x:auto;scrollbar-width:thin}
    .ptl-filtros-fases .ptl-filtro{flex-shrink:0}
    ...
    return `
      <div class="ptl-lista-header">
        ...
        <div class="ptl-filtros ptl-filtros-fases">
          ...
          ${filtroBtn("03_ENVIO", "03-ENVIO PTO")}
          ${filtroBtn("04_SEGUIMIENTO", "04-SEGUIMIENTO PTO")}
          ${filtroBtn("05_RESOLUCION", "05-RESOLUCION PTO")}
  ...
  function vistaFicha(comu, vecinos, datalists, token, todosExpedientes, reciencreado) {
    ...
    if (fase === "ZZ_RECHAZADO") {
      ...
      // Fases con email asociado: 01_SOLICITUD, 03_ENVIO, 04_SEGUIMIENTO
      ...
      const labelMail = numEnviosFase === 0
        ? `📧 Enviar mail ${def.codigo}-${(def.nombreLargo || def.nombre || '').toUpperCase()}`
        : `📧 Enviado ${numEnviosFase} ${numEnviosFase === 1 ? 'vez' : 'veces'}`;

      // Texto indicador (círculo rojo): "01-SOLICITUD · 27/04/26"
      const fechaActual = fechaHito(comu, fase);
      const fechaTxt = fechaActual ? ` · ${fmtFecha(fechaActual)}` : '';
      const labelFaseActual = `${def.codigo}-${(def.nombreLargo || def.nombre || '').toUpperCase()}${fechaTxt}`;

      // Texto botón paso a fase siguiente: "→ Paso a 02-PTE VISITA"
      ...
      accionHtml = `<div class="ptl-next-action">
        ...
        ${tienePlantilla ? `
          <button type="button" class="ptl-btn ptl-btn-secondary ptl-btn-sm"
            onclick="ptlAbrirModalMail('${esc(fase)}', '${esc(comu.ccpp_id)}')">${labelMail}</button>
        ` : ''}
    ...
    return `
      ...
      <form id="ptl-ficha-form" data-id="${esc(comu.ccpp_id)}" onsubmit="return false">
        ...
        <div class="ptl-card">
          ...
          <div class="ptl-form-grid">
            ${inp("tiempo_previsto", comu.tiempo_previsto, { type: "number", formato: "dias", col: 4, label: "Tiempo previsto (días)" })}
            ${inp("tiempo_real",     comu.tiempo_real,     { type: "number", formato: "dias", col: 4, label: "Tiempo real (días)" })}
            <div class="col-4">
      ...
      <script>
          ...
          try {
            ...
            btn.onclick = async () => {
              ...
              try {
                ...
                let msg = '✓ Email registrado (envío SIMULADO).\\nEnvíos totales: ' + dd.envios;
                if (dd.fase_actualizada) msg += '\\n\\n⚠ Se alcanzó el máximo. Expediente pasado a ' + dd.fase_actualizada + '.';
                alert(msg);
                ptlCerrarModalMail();
        ...
        // Si el expediente acaba de crearse, preguntar si enviar el primer mail de la fase 01
        ${reciencreado ? `
        setTimeout(() => {
          if (confirm('¿Enviar email solicitando aprobación de presupuesto en Acta de Asamblea?')) {
  ...
  app.get("/presupuestos/expediente", async (req, res) => {
    ...
    try {
      ...
      const reciencreado = req.query.creado === "1";
  ...
  // body: id, fase, asunto, mensaje, destinatario, adjuntos
    ...
    try {
      ...
      if (plantilla.max_envios > 0 && nuevoCount > plantilla.max_envios) {
        return res.status(400).json({
          error: `Se alcanzó el máximo de envíos (${plantilla.max_envios}). El expediente debería pasar a ZZ_DESCARTADO.`
        });
      }
      ...
      // Actualizar contador y fecha en la comunidad
      ...
      // Si hemos llegado al tope con este envío, pasar a ZZ_DESCARTADO
      let fasePasoA = null;
      if (plantilla.max_envios > 0 && nuevoCount >= plantilla.max_envios) {
        comu.fase_presupuesto = "ZZ_DESCARTADO";
        fasePasoA = "ZZ_DESCARTADO";
      }
