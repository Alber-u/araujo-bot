...
<html lang="es"><head>
      ...
      return `
        <div class="ptl-card" style="margin-bottom:16px">
          ...
          <form method="POST" action="${urlT(token, "/presupuestos/plantillas/guardar")}" style="padding:12px">
            ...
            <label style="font-size:13px;display:block;margin-bottom:12px">
              <div style="margin-bottom:4px;font-weight:600">CCO (con copia oculta) — opcional</div>
              <input type="text" name="cco" value="${esc(p.cco || '')}" maxlength="300"
                placeholder="hasta 3 emails separados por coma (sin acentos)"
                pattern="[A-Za-z0-9._%+\\-]+@[A-Za-z0-9.\\-]+\\.[A-Za-z]{2,}(\\s*,\\s*[A-Za-z0-9._%+\\-]+@[A-Za-z0-9.\\-]+\\.[A-Za-z]{2,}){0,2}"
                style="width:100%;padding:6px;border:1px solid var(--ptl-gray-200);border-radius:4px"/>
              <div style="font-size:11px;color:var(--ptl-gray-500);margin-top:2px">Si se rellena, esos destinatarios reciben copia oculta de cada envío de esta plantilla</div>
            </label>
  ...
  app.post("/presupuestos/plantillas/guardar", async (req, res) => {
    ...
    try {
      ...
      // Validar CCO: hasta 3 emails separados por coma, sin acentos
      if (!esListaEmailsValida(datos.cco, 3)) {
        return sendError(res, "CCO no válido. Debe contener hasta 3 direcciones de email válidas (separadas por coma, sin acentos ni espacios).");
      }
