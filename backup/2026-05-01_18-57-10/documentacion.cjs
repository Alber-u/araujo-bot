...
module.exports = function (app) {
  ...
  const RANGO_EXPEDIENTES = "pisos!A:AB";
      ...
      <div class="ptl-card ptl-vec-card" data-direccion="${esc(comu.direccion)}" data-comunidad="${esc(comu.comunidad)}">
        ...
        <script>
          (function() {
            ...
            // (guardar/borrar piso, marcar documento, cambiar modo). Limpia
            // el flag dirty para que el navegador NO muestre el diálogo de
            // "¿abandonar página?".
            function recargar() {
            ...
            function abrirMenuDoc(btn) {
              ...
              // Cerrar al pulsar fuera, al hacer scroll o al redimensionar
              setTimeout(() => {
                document.addEventListener('click', cerrarFueraMenuDoc, { once: true });
              }, 0);
              window.addEventListener('scroll', cerrarMenuDoc, { once: true, capture: true });
              window.addEventListener('resize', cerrarMenuDoc, { once: true });
