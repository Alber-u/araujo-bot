  ...
  function cajitaVecinosHtml(comu, vecinosBase, expedientes, token, esc) {
    ...
    const filasHtml = filas.map((f, idx) => {
      ...
      return `<tr class="ptl-vecino-row" data-tlf="${esc(f.telefono)}">
        <td><input type="text" class="ptl-inline-input" value="${esc(f.vivienda)}" onchange="ptlDoc.actualizar('${esc(f.telefono)}','vivienda',this.value)"/>${huerfano}</td>
        <td><input type="text" class="ptl-inline-input" value="${esc(f.nombre)}" onchange="ptlDoc.actualizar('${esc(f.telefono)}','nombre',this.value)"/></td>
        <td class="ptl-num-cell">${esc(tlfFmt)}</td>
        <td>${badgeEstadoVecino(f.estado, esc)}</td>
        <td>${resumenDocs}</td>
        <td><button type="button" class="ptl-btn-mini" onclick="ptlDoc.toggleAcord('${acordeonId}')">✏ Rellenar</button></td>
      </tr>
      ${acordeon}`;
    }).join("");
    ...
    const filaNueva = `
      <tr class="ptl-vecino-nuevo">
        <td><input type="text" id="vec-new-vivienda" class="ptl-inline-input" placeholder="Vivienda"/></td>
        <td><input type="text" id="vec-new-nombre" class="ptl-inline-input" placeholder="Nombre"/></td>
        <td><input type="tel" id="vec-new-telefono" class="ptl-inline-input" placeholder="Teléfono"/></td>
        <td colspan="2"><em style="color:var(--ptl-gray-500);font-size:12px">Nuevo vecino</em></td>
        <td><button type="button" class="ptl-btn-mini ptl-btn-primary" onclick="ptlDoc.crearVecino()">+ Añadir</button></td>
      </tr>`;
    ...
    const cssInline = `
      <style>
        .ptl-inline-input{width:100%;border:1px solid var(--ptl-gray-200);border-radius:4px;padding:4px 6px;font-size:13px;background:#fff}
        ...
        .ptl-btn-mini.ptl-btn-primary{background:#2c5282;color:#fff;border-color:#2c5282}
        .ptl-btn-mini.ptl-btn-primary:hover{background:#1e3a5f}
