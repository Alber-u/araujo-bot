...
<html lang="es"><head>
  ...
  const CSS = `
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .ptl-next-action.ptl-next-action-grid{background:#C7D2FE;border-color:#A5B4FC;display:grid;grid-template-columns:minmax(0,1fr) auto auto;align-items:stretch;padding:2px 8px;gap:6px;min-width:0;margin-bottom:6px;flex-wrap:initial}
  ...
  app.post("/presupuestos/expediente/avanzar", async (req, res) => {
    ...
    try {
      ...
      if (def && def.siguiente) {
        ...
        // fecha_visita_pto YA NO se rellena al entrar en 02_VISITA: se rellena manualmente
        // desde el mini-bloque "FECHA VISITA" cuando se realiza la visita real.
