...
<html lang="es"><head>
  ...
  const CSS = `
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    /* Variante grid (3 zonas: izq texto / centro botón cuadrado / der botones apilados) */
    .ptl-next-action-grid{display:grid;grid-template-columns:1fr auto auto;align-items:stretch;padding:6px 10px;gap:10px;flex-wrap:nowrap}
    .ptl-next-action-grid .ptl-na-left{display:flex;align-items:center;gap:8px;min-width:0}
    .ptl-next-action-grid .ptl-na-right{display:flex;flex-direction:column;gap:4px;justify-content:center}
    .ptl-next-action-grid .ptl-na-right .ptl-btn{white-space:nowrap}
    /* Botón cuadrado de mail */
    .ptl-btn-mail-square{display:flex;flex-direction:column;align-items:center;justify-content:center;gap:1px;min-width:88px;padding:6px 10px;border:1.5px solid #C7D2FE;background:white;color:#4F46E5;border-radius:8px;font-weight:600;cursor:pointer;font-family:inherit;line-height:1.15;transition:all .15s}
    .ptl-btn-mail-square:hover{background:#4F46E5;border-color:#4F46E5;color:white}
    .ptl-btn-mail-square .ico{font-size:18px;margin-bottom:1px}
    .ptl-btn-mail-square .ln{font-size:10.5px;display:block}
    .ptl-btn-mail-done{background:#F0FDF4;border-color:#A7F3D0;color:#047857;cursor:default}
    .ptl-btn-mail-done:hover{background:#F0FDF4;border-color:#A7F3D0;color:#047857}
    ...
    return `
      <div class="ptl-lista-header">
        ...
        <div class="ptl-filtros ptl-filtros-fases">
          ...
          ${filtroBtn("02_VISITA", "02-PTE VISITA", "ptl-fase-activa")}
  ...
  function vistaFicha(comu, vecinos, datalists, token, todosExpedientes, reciencreado) {
    ...
    if (fase === "ZZ_RECHAZADO") {
      ...
      accionHtml = `<div class="ptl-next-action ${urgCls}">
        <div class="ico">⚖</div>
        <div style="flex:1"><div class="text">Seguimiento en curso · ¿Decisión del cliente?</div>
      ...
      // Botón mail cuadrado: solo si la plantilla está activa y aún no se ha enviado nada
      let btnMailHtml = '';
      if (tienePlantilla && numEnviosFase === 0) {
        btnMailHtml = `<button type="button" class="ptl-btn-mail-square"
        ...
        // Ya activado: placeholder vacío para mantener la rejilla (mantiene el ancho)
        btnMailHtml = `<div class="ptl-btn-mail-square ptl-btn-mail-done" title="Envíos automáticos en curso">
          <span class="ico">✓</span>
          <span class="ln">Mail</span>
          <span class="ln">automático</span>
          <span class="ln">activo</span>
        </div>`;
      }
