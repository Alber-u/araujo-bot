...
module.exports = function (app) {
  ...
  const RANGO_EXPEDIENTES = "expedientes!A:Z";
  ...
  async function guardarPiso(comu, { codigoPisoBruto, nombreBruto, telefonoBruto, _rowIndex }) {
    ...
    if (_rowIndex && telefonoViejo && telefonoViejo !== telefono) {
      ...
      if (exp) {
        ...
        while (filaExp.length < 26) filaExp.push("");
        filaExp[0] = telefono;
        await sheets.spreadsheets.values.update({
          spreadsheetId: SHEET_ID,
          range: `expedientes!A${exp._rowIndex}:Z${exp._rowIndex}`,
          valueInputOption: "RAW",
        ...
        <script>
          (function() {
            ...
            function pintarAcordeon(filaPiso, contenedor) {
              ...
              if (!exp) {
                cab = '<div class="ptl-vec-ac-cab ptl-vec-ac-sinexp">'
                  ...
                  + '<span class="ptl-vec-ac-cab-info">La presentación no se ha enviado todavía a este vecino.</span>'
                  + '</div>';
              } else {
                cab = '<div class="ptl-vec-ac-cab">'
                  + badgeEstado(exp.estado_expediente)
                  + '<span class="ptl-vec-ac-cab-counter">'+recibidos.length+'/'+(recibidos.length+pendientes.length)+' docs</span>'
                  + (exp.fecha_inicio ? '<span class="ptl-vec-ac-cab-fecha">Inicio: '+esc(exp.fecha_inicio)+'</span>' : '')
                  + (exp.fecha_ultimo_contacto ? '<span class="ptl-vec-ac-cab-fecha">Último contacto: '+esc(exp.fecha_ultimo_contacto)+'</span>' : '')
                  + (exp.fecha_limite_documentacion ? '<span class="ptl-vec-ac-cab-fecha">Plazo: '+esc(exp.fecha_limite_documentacion)+'</span>' : '')
                  + '</div>';
              }

              const docs = docsUniversal.map(d => {
                const recib = recibidosSet.has(d.codigo);
                const cls = recib ? 'ptl-vec-doc-recibido' : 'ptl-vec-doc-pendiente';
                const icono = recib ? '📎' : '⬆️';
                return '<div class="ptl-vec-doc-fila">'
                  + '<button type="button" class="ptl-vec-doc-btn '+cls+'" '
                  + 'data-codigo="'+esc(d.codigo)+'" data-estado="'+(recib ? 'recibido_archivo' : 'pendiente')+'" '
                  + 'title="Disponible próximamente: subida y gestión de archivos">'+icono+'</button>'
              ...
              contenedor.innerHTML = cab
                ...
                + '<div class="ptl-vec-ac-aviso">⚠ La gestión de archivos (subir / ver / descargar) estará disponible en la próxima actualización.</div>';
            }
            ...
            tbody.addEventListener('click', e => {
              ...
              if (docBtn) {
                alert('La gestión de archivos estará disponible en la próxima actualización.');
              }
            });
  ...
  console.log("[documentacion] Módulo cargado. Rutas: /documentacion/expediente, /documentacion/piso/guardar, /documentacion/piso/borrar, /documentacion/ccpp/modo");
