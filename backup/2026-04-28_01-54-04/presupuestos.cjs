...
module.exports = function (app) {
  ...
  const PTO_FASES = {
    ...
    "ZZ_DESCARTADO":    { codigo: "ZZ", nombre: "Descartado",  color: "rojo",     siguiente: null,              accionLabel: "Descartado",           plantilla: null },
  };
  ...
  function vistaFicha(comu, vecinos, datalists, token, todosExpedientes, reciencreado) {
    ...
    if (fase === "ZZ_RECHAZADO") {
      ...
      const labelMail = numEnviosFase === 0
        ? `📧 Enviar mail ${def.codigo}-${def.nombre}`
      ...
      accionHtml = `<div class="ptl-next-action">
        ...
        <form method="POST" action="${urlT(token, "/presupuestos/expediente/descartar")}" style="display:inline">
          <input type="hidden" name="id" value="${esc(comu.ccpp_id)}"/>
          <button type="submit" class="ptl-btn ptl-btn-danger ptl-btn-sm" onclick="return confirm('¿Descartar este expediente? Pasará a ZZ-DESCARTADO y no podrá enviarse más.')">✕ Descartar</button>
        </form>
      </div>`;
    }
