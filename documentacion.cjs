...
module.exports = function (app) {
          ...
          function abrirMenuBot(btn){
            ...
            menu.addEventListener('click', async function(ev){
              ...
              if(b.dataset.adjuntarCara!==undefined){
                ...
                var fdc=new URLSearchParams();
                fdc.append('ccpp_clave',direccion); fdc.append('vivienda',vivienda); fdc.append('codigo',caraCode); fdc.append('url',enlace);
                if(token) fdc.append('token',token);
                try{
                  ...
                  if(dp){ dp.botDocs=Array.isArray(dp.botDocs)?dp.botDocs:[]; var nowc=new Date().toISOString(); var fc=false;
                    dp.botDocs.forEach(function(x){ if(x.code===caraCode){ x.estado='OK'; x.url=enlace; x.fecha=nowc; fc=true; } });
                    if(!fc) dp.botDocs.push({code:caraCode,estado:'OK',url:enlace,fecha:nowc}); }
                  var idx2=indexBotDocs(dp); var e2=estadoSwitchBot(code,idx2);
                  btn.textContent=TXT_BOT[e2]||'F'; btn.className='ptl-bot-sw ptl-bot-sw-'+(COL_BOT[e2]||'rojo');
                  btn.dataset.urlDel=urlCaraBot(code,idx2,0); btn.dataset.urlDet=urlCaraBot(code,idx2,1);
                  refrescarContadores();
                ...
                enlace=String(enlace).trim();
                if(!/^https?:\/\//i.test(enlace)){ alert('El enlace debe empezar por http:// o https://'); return; }
                var fda=new URLSearchParams();
                fda.append('ccpp_clave',direccion); fda.append('vivienda',vivienda); fda.append('codigo',code); fda.append('url',enlace);
                if(token) fda.append('token',token);
                try{
                  ...
                  if(dp){ dp.botDocs=Array.isArray(dp.botDocs)?dp.botDocs:[]; var nowa=new Date().toISOString(); var f2=false;
                    dp.botDocs.forEach(function(x){ if(x.code===code){ x.estado='OK'; x.url=enlace; x.fecha=nowa; f2=true; } });
                    if(!f2) dp.botDocs.push({code:code,estado:'OK',url:enlace,fecha:nowa}); }
                  refrescarContadores();
                }catch(err){ alert('No se pudo adjuntar: '+(err.message||err)); }
                return;
              }
  ...
  app.post("/documentacion/bot/adjuntar", async (req, res) => {
    ...
    try {
      ...
      const now = new Date().toISOString();
      if (rowIndex > 0) {
        // Actualiza fecha (F), url (G), origen (H), estado (I) de la fila existente.
