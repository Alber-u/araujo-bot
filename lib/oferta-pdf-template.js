...
// Logo y sello: si llega un PNG en assets se usa (data-URI). Si no,
// fallback a los SVG vectoriales. Así las pruebas locales sin assets
// siguen renderizando sin romperse.
function renderLogo(p, opts = {}) {
  const png = p.assets?.logoPng;
  if (png) {
    const cls = opts.variant === "inverso" ? "logo-img inv" : "logo-img";
    return `<img src="${png}" class="${cls}" alt="Araujo"/>`;
  }
  return logoSvg(opts);
}
...
/* v3.6 · logo PNG (cuando hay assets). Sin filtros para respetar
   los colores reales del PNG embebido en /assets. */
.cover .marca .logo .logo-img,
.cover .marca .logo .logo-img.inv{
  width:100%;height:100%;display:block;object-fit:contain;
}
.pageHead .marca .logo{width:28px;height:28px;display:block}
.pageHead .marca .logo .logo-img{
  width:100%;height:100%;display:block;object-fit:contain;
}
/* v3.6 · sello+firma PNG · grande en la zona "Por Araujo" */
.accept .sign.empresa .sello{
  position:absolute;right:-2mm;top:-4mm;width:90px;height:90px;opacity:.95;
}
.accept .sign.empresa .sello .sello-img{
  width:100%;height:100%;display:block;object-fit:contain;
}
.accept .watermark .sello-img{
  width:100%;height:100%;display:block;object-fit:contain;opacity:.07;
}
.cover .marca .sub{
...
.cover .issue{position:absolute;left:18mm;top:142mm;color:#fff;z-index:3}
.cover .issue .l{font-size:8pt;font-weight:600;letter-spacing:.23em;color:rgba(255,255,255,.58)}
.cover .issue .n{
  font-family:var(--serif);font-weight:900;
  font-size:70pt;letter-spacing:-3.8px;line-height:.85;margin-top:5px;
}
...
.scope .descripcion{
  font-family:var(--sans);font-size:10pt;line-height:1.55;color:var(--text);
}
.scope .descripcion p{
  margin:0 0 10pt 0;
}
.scope .descripcion p:first-child{
  font-family:var(--serif);font-size:13pt;line-height:1.4;color:var(--navy);
  letter-spacing:-.1px;margin-bottom:14pt;
}
.scope .descripcion ul{
  list-style:none;padding:0;margin:0 0 10pt 0;
}
.scope .descripcion li{
  position:relative;padding-left:14pt;margin-bottom:5pt;font-size:9.5pt;line-height:1.5;color:var(--text);
}
...
function htmlAceptacion(p) {
  ...
  return `
    <section class="page accept">
      <div class="watermark">${renderSello(p)}</div>
