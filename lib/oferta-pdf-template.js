...
/* v3.9 · páginas de continuación del alcance · ancho completo, sin
   panel lateral ni capítulo; el primer párrafo NO va destacado. */
.scope.cont .body{position:absolute;left:18mm;right:18mm;top:46mm;width:auto}
.scope .descripcion.cont p:first-child{
  font-family:var(--sans);font-size:10pt;line-height:1.55;color:var(--text);margin:0 0 10pt 0;
}
...
/* ════════════════════════════════════════════════════════════════
   PÁGINA 4 · ACEPTACIÓN
   ════════════════════════════════════════════════════════════════ */
/* v3.9 · zona superior en FLUJO: la tira de datos puede crecer (obra
   con título largo) y los párrafos legales se colocan debajo sin
   pisarse. importe/firmas/footer siguen anclados abajo. */
.accept .docTop{position:absolute;left:18mm;right:18mm;top:114mm}
.accept .id{
  display:grid;grid-template-columns:repeat(4,1fr);
...
.accept .legal{font-size:10pt;line-height:1.6;color:var(--text);letter-spacing:.005em;margin-top:18px}
...
function fmtPag(n) { return String(n).padStart(2, "0"); }
function htmlPageFoot(p, idx, total = 4) {
  return `
    <div class="pageFoot">
      <div><strong>${esc(p.empresa.nombre)}</strong> · ${esc(p.empresa.email)}</div>
      <div>${esc(fmtPag(idx))} / ${esc(fmtPag(total))}</div>
    </div>`;
}
...
// v3.9 · El alcance se trocea en varias páginas cuando es largo, en
// vez de encogerse. La estimación de altura es determinista y replica
// EXACTAMENTE la del front (src/components/pdf/alcance.js) para que la
// preview y el PDF troceen igual. Mantener ambas sincronizadas.
function textoAlcance(p) {
  return (p.alcanceTexto || p.alcanceIntro || "").trim()
    || "Trabajos de instalación según las especificaciones detalladas en este documento.";
}
function itemsAlcance(texto) {
  const lineas = String(texto || "").split(/\r?\n/).map(s => s.trim()).filter(Boolean);
  return lineas.map(l => {
    const m = l.match(/^([-—•*]+|\d+[.)])\s*[.)\-]?\s*(.*)$/);
    if (m && m[2]) return { tipo: "li", text: m[2] };
    return { tipo: "p", text: l };
  });
}
function alturaItemAlcance(it, widthMm, esLead) {
  if (it.tipo === "li") {
    const cpl = Math.max(8, Math.floor((widthMm - 6) / 1.95));
    const lines = Math.max(1, Math.ceil(String(it.text || "").length / cpl));
    return lines * 5.03 + 1.76;
  }
  if (esLead) {
    const cpl = Math.max(8, Math.floor(widthMm / 2.7));
    const lines = Math.max(1, Math.ceil(String(it.text || "").length / cpl));
    return lines * 6.42 + 4.94;
  }
  const cpl = Math.max(8, Math.floor(widthMm / 2.05));
  const lines = Math.max(1, Math.ceil(String(it.text || "").length / cpl));
  return lines * 5.47 + 3.53;
}
function paginarAlcance(texto) {
  const items = itemsAlcance(texto);
  const PRIMERA = { width: 108, alto: 155 };
  const CONT = { width: 174, alto: 225 };
  const paginas = [];
  let actual = [], alto = 0;
  for (let i = 0; i < items.length; i++) {
    const g = paginas.length === 0 ? PRIMERA : CONT;
    const hb = alturaItemAlcance(items[i], g.width, i === 0);
    if (alto + hb > g.alto && actual.length > 0) {
      paginas.push(actual); actual = []; alto = 0;
    }
    actual.push(items[i]); alto += hb;
  }
  if (actual.length) paginas.push(actual);
  return paginas.length ? paginas : [[]];
}
// Agrupa los ítems de una página en <p>/<ul> consecutivos.
function cuerpoAlcanceHtml(items) {
  let out = "";
  let buffer = [];
  const flush = () => { if (buffer.length) { out += `<ul>${buffer.join("")}</ul>`; buffer = []; } };
  for (const it of items) {
    if (it.tipo === "li") buffer.push(`<li>${esc(it.text)}</li>`);
    else { flush(); out += `<p>${esc(it.text)}</p>`; }
  }
  flush();
  return out;
}

// Devuelve { html, nPaginas } para poder numerar el resto del documento.
function htmlAlcance(p, total) {
  const paginas = paginarAlcance(textoAlcance(p));
  const secciones = paginas.map((items, idx) => {
    const pageNum = 2 + idx;
    const cuerpo = cuerpoAlcanceHtml(items);
    if (idx === 0) {
      return `
    <section class="page scope">
      ...
      <div class="body">
        <div class="descripcion">${cuerpo}</div>
      </div>
      <div class="visual">
        ${p.theme.blueprintSvg}
        <div class="cap">ESPECIALIDAD<b>${esc(p.theme.label)}</b></div>
      </div>
      ${htmlPageFoot(p, pageNum, total)}
    </section>`;
    }
    return `
    <section class="page scope cont">
      ${htmlPageHead(p, "ALCANCE · CONT.")}
      <div class="body">
        <div class="descripcion cont">${cuerpo}</div>
      </div>
      ${htmlPageFoot(p, pageNum, total)}
    </section>`;
  });
  return { html: secciones.join(""), nPaginas: paginas.length };
}

function htmlInversion(p, pageNum = 3, total = 4) {
...
function htmlAceptacion(p, pageNum = 4, total = 4) {
  const o = p.oferta;
  ...
  return `
    <section class="page accept">
      ...
      <div class="docTop">
        ...
        <div class="legal">Mediante la firma del presente documento, el <strong>CLIENTE</strong> acepta el presupuesto identificado y autoriza el inicio del trabajo conforme a las condiciones descritas en la oferta.</div>
        <div class="legal">El importe total acordado asciende a <strong>${esc(o.total)} (IVA incluido)</strong>. La forma de pago se acordará previamente al inicio. Cualquier variación derivada de imprevistos en obra será notificada al CLIENTE y requerirá aprobación expresa.</div>
      </div>
      ...
      <div class="corpFooter">
        ...
        <div class="pn">${fmtPag(pageNum)}<span>DE ${fmtPag(total)}</span></div>
      </div>
    </section>`;
}
...
function renderPresupuestoHtml(presupuesto) {
  ...
  // El alcance puede ocupar varias páginas. Total = portada + alcance(N)
  // + inversión + aceptación, para numerar bien los pies de página.
  const nScope = paginarAlcance(textoAlcance(p)).length;
  const total = 3 + nScope;
  const alcance = htmlAlcance(p, total);
  return `<!doctype html>
<html lang="es">
<head>
  <meta charset="utf-8"/>
...
<body>
  <div class="pdf-root">
    ${htmlCover(p)}
    ${alcance.html}
    ${htmlInversion(p, 2 + nScope, total)}
    ${htmlAceptacion(p, 3 + nScope, total)}
  </div>
</body>
</html>`;
}
