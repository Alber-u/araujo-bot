...
(function(){
  ...
  if(list){ list.querySelectorAll('[data-p5act]').forEach(function(el){ el.addEventListener('click',function(){ var a=el.getAttribute('data-p5act'); if((a==='IMPORTAR CATASTRO'||a==='IMPORTAR IMAGENES')&&window.__PLAN5_SAVED__&&window.__PLAN5_SAVED__.estado==='cerrado'){alert('Presupuesto cerrado: abrelo con el candado para poder importar.');return;} if(a==='IMPRIMIR PRESUPUESTO'){ window.open('/plan5/presupuesto'+q(),'_blank'); list.hidden=true; } if(a==='IMPORTAR CATASTRO'){ list.hidden=true; (function(){ var g=function(id){var e=document.getElementById(id);return e?(e.value||''):'';}; var p=[]; p.push('tipovia='+encodeURIComponent(g('f_tipovia'))); p.push('calle='+encodeURIComponent(g('f_direccion'))); p.push('poblacion='+encodeURIComponent(g('f_poblacion'))); p.push('cp='+encodeURIComponent(g('f_cp'))); if(tk)p.push('token='+encodeURIComponent(tk)); fetch('/plan5/catastro?'+p.join('&')).then(function(r){return r.json();}).then(function(j){ if(window.catImport)window.catImport(j); else alert('No se pudo cargar la lista.'); }).catch(function(e){ alert('Error consultando el Catastro: '+e.message); }); })(); } }); }); }
})();
`;
...
const _P5_REVISION = "Rev-20.0 24/06/2026";
