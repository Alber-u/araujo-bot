...
(function(){
  ...
  if(list){ list.querySelectorAll('[data-p5act]').forEach(function(el){ el.addEventListener('click',function(){ var a=el.getAttribute('data-p5act'); if(a==='IMPRIMIR PRESUPUESTO'){ window.open('/plan5/presupuesto'+q(),'_blank'); list.hidden=true; } if(a==='IMPORTAR CATASTRO'){ list.hidden=true; (function(){ var g=function(id){var e=document.getElementById(id);return e?(e.value||''):'';}; var p=[]; p.push('tipovia='+encodeURIComponent(g('f_tipovia'))); p.push('calle='+encodeURIComponent(g('f_direccion'))); p.push('poblacion='+encodeURIComponent(g('f_poblacion'))); p.push('cp='+encodeURIComponent(g('f_cp'))); if(tk)p.push('token='+encodeURIComponent(tk)); fetch('/plan5/catastro?'+p.join('&')).then(function(r){return r.json();}).then(function(j){ if(window.catImport)window.catImport(j); else alert('No se pudo cargar la lista.'); }).catch(function(e){ alert('Error consultando el Catastro: '+e.message); }); })(); } }); }); }
})();
`;
...
const _P5_REVISION = "Rev-19.5 24/06/2026";
...
module.exports = function (app) {
  ...
  // Coordenadas (centroide) de la parcela, para situar el plano WMS de fondo del croquis.
  async function _p5catCoordParcela(prov, muni, rc14){
    try{
      var url = "https://ovc.catastro.meh.es/ovcservweb/OVCSWLocalizacionRC/OVCCoordenadas.asmx/Consulta_CPMRC?Provincia=" + encodeURIComponent(prov) + "&Municipio=" + encodeURIComponent(muni) + "&SRS=EPSG:25830&RC=" + encodeURIComponent(rc14);
      var xml = await _p5catFetch(url);
      var x = parseFloat(_p5catTag(xml, "xcen")), y = parseFloat(_p5catTag(xml, "ycen"));
      if (isNaN(x) || isNaN(y)) return null;
      return { srs: "EPSG:25830", cx: x, cy: y };
    } catch(e){ return null; }
  }
