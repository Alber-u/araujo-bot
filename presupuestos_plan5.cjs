...
module.exports = function (app) {
  ...
  app.get("/plan5/precios", async function (req, res) {
    ...
    var inj = "window.__PRECIOS__=" + JSON.stringify(datos) + ";window.__PLAN5_VOLVER__=" + JSON.stringify(volver) + ";window.__PLAN5_TOKEN__=" + JSON.stringify(token) + ";";
    var theme = "";
    try { theme = getThemeCss() || ""; } catch (e) { theme = ""; }
  ...
  app.post("/plan5/precios/guardar", async function (req, res) {
    ...
    try {
      ...
      var n = numEs(b.precio);
      await sh().spreadsheets.values.update({
        spreadsheetId: sid(), range: "plan5_precios!D" + row,
        valueInputOption: "RAW", requestBody: { values: [[ n == null ? "" : n ]] },
      });
      res.json({ ok: true });
    } catch (e) {
