...
const _P5_REVISION = "Rev-19.9 24/06/2026";
...
module.exports = function (app) {
  ...
  app.get("/plan5/presupuesto", async function (req, res) {
    ...
    try {
      ...
      try {
        if (savedExp && c && P().getImagenesExpediente) { savedExp.imagenes = await P().getImagenesExpediente(c.tipo_via, c.direccion); }
      } catch (e) { console.error("[plan5] imagenes expediente:", e.message); }
