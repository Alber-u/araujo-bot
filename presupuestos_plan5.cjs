...
const _P5_REVISION = "Rev-19.14 24/06/2026";
...
function _p5paginaImagenes(R, meta, cuadro, saved){
  ...
  for (var k = 0; k < 11; k++){
    ...
    var inner = idq
      ? ("<img class=\"p5img\" decoding=\"sync\" loading=\"eager\" src=\"/plan5/imagen?id=" + idq + "&n=" + nn + tkq + "\" style=\"max-width:100%;max-height:100%;object-fit:contain\" onerror=\"this.style.display='none';var p=this.parentNode.querySelector('.p5ph');if(p)p.style.display='block';\"><span class=\"p5ph\" style=\"display:none;color:#b6c2d2;font-size:10px\">" + nn + ".png</span>")
...
<div class="p5toolbar">
  ...
  <button class="p5btn" onclick="p5imprimir()">&#128424; Imprimir / Guardar PDF</button><script>(function(){function init(){var L=[].slice.call(document.querySelectorAll('.p5img'));var b=document.querySelector('.p5btn');if(!b)return;var total=L.length,hechas=0;function upd(){if(hechas>=total){b.disabled=false;b.innerHTML='&#128424; Imprimir / Guardar PDF';}else{b.disabled=true;b.textContent='Cargando fotos... '+hechas+'/'+total;}}if(total===0){b.disabled=false;return;}upd();L.forEach(function(im){var ok=function(){hechas++;upd();};if(im.complete){ok();}else{im.addEventListener('load',ok,{once:true});im.addEventListener('error',ok,{once:true});}});}if(document.readyState!=='loading'){init();}else{document.addEventListener('DOMContentLoaded',init);}})();function p5imprimir(){var b=document.querySelector('.p5btn');if(b&&b.disabled)return;setTimeout(function(){window.print();},300);}</script>
</div>
