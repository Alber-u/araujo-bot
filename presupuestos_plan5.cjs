...
function _p5memoria(R, meta, saved){
  ...
  var _peinesAct = (function(){ var grupos=[]; peines.forEach(function(pe){ if(!pe || !(pe.maIE||pe.maEng)) return; var ie = String(pe.maIE||"").toUpperCase().indexOf("EXT")>=0 ? "exterior" : "interior"; var eng = String(pe.maEng||"").toUpperCase().indexOf("BA\u00d1O")>=0 ? "ba\u00f1os" : "cocinas"; var pu = pe.puerta ? String(pe.puerta).trim() : ""; var g=null; for(var i=0;i<grupos.length;i++){ if(grupos[i].ie===ie && grupos[i].eng===eng){ g=grupos[i]; break; } } if(!g){ g={ie:ie,eng:eng,puertas:[]}; grupos.push(g); } if(pu) g.puertas.push(pu); }); return grupos.map(function(g){ var n=g.puertas.length; if(n===0){ return " El montante es "+g.ie+" y engancha en "+g.eng+"."; } if(n===1 && !/(^|\s)(Y|E)\s|[,\/+&]/i.test(g.puertas[0])){ return " El de la PUERTA "+g.puertas[0]+" es "+g.ie+" y engancha en "+g.eng+"."; } var adj = (g.ie==="exterior") ? "exteriores" : "interiores"; return " Los de las PUERTAS "+_p5listaES(g.puertas)+" son "+adj+" y enganchan en "+g.eng+"."; }).join(""); })();
