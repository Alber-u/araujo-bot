...
module.exports = function (app) {
  ...
  app.get(["/plan5", "/plan5/toma-datos"], async function (req, res) {
    ...
    // Modelos de grupo de presion: salen de plan5_precios (concepto "2x...Kw",
    // detalle = modelo). Asi las filas que anada Guille aparecen solas.
    var _gpMods = [];
    try {
      var _pr = await leerPrecios();
      for (var _i = 0; _i < _pr.length; _i++) {
        var _c = String(_pr[_i].concepto || "").trim();
        if (/^2x[0-9]/i.test(_c) && String(_pr[_i].tipo || "").trim()) {
          _gpMods.push({ pot: _c, mod: String(_pr[_i].tipo).trim() });
        }
      }
    } catch (e) { _gpMods = []; }
    const injGP = ";window.__PLAN5_GPMODELOS__=" + JSON.stringify(_gpMods) + ";";
    let theme = "";
    ...
    const html = TOMA_DATOS_HTML.replace("__PLAN5_REV__", _P5_REVISION).replace("__PLAN5_COND__", _P5_CONDICIONES)
      ...
      .replace("/*__PLAN5_SAVED__*/", inj + injGP)
