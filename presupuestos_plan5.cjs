...
const _P5_REVISION = "Rev-19.45 25/06/2026";
...
module.exports = function (app) {
  ...
  app.post("/plan5/cerrar", async function (req, res) {
    ...
    try {
      ...
      // beneficio_previsto NO se escribe: es columna calculada por el Sheet.
      try {
        var _Pf  = P();
        ...
        if (_cdr && _Pf.buscarComunidadPorId && _Pf.actualizarCampoComunidad) {
          ...
          if (_comuF && _comuF._rowIndex != null) {
            ...
            var _matPrev = (Number(_cdr.cMat) || 0) + (Number(_cdr.cAlb) || 0) + (Number(_cdr.cGp) || 0);
            await _Pf.actualizarCampoComunidad(_riF, "pto_total",          Number(_cdr.totP5Iva) || 0);
            await _Pf.actualizarCampoComunidad(_riF, "mano_obra_previsto", Number(_cdr.cMo)      || 0);
            await _Pf.actualizarCampoComunidad(_riF, "material_previsto",  _matPrev);
            await _Pf.actualizarCampoComunidad(_riF, "tiempo_previsto",    Number(_cdr.tEjec)    || 0);
          }
        }
      } catch (eVol) {
