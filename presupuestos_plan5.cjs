...
const OBRA_DEFAULT = {
  ...
  alimentacion: {
    ...
    tiempoFont: [ { hasta: 6.5, dias: 1 }, { hasta: 15, dias: 1.5 } ],
    tiempoEnt:  [ { hasta: 6.5, dias: 1.5 }, { hasta: 15, dias: 3 } ],
    tiempoViga: [ { hasta: 6.5, dias: 0.5 }, { hasta: 15, dias: 1 } ],
  },
};
...
function calcAlimentacion(nsum, tipo, longAli, montaje, codosTermo, llaves, dosBaterias, precios, obra) {
  ...
  const dF = diasPorTramo(O.tiempoFont, longAli);
  const dE = (montaje === "ENTERRADO") ? diasPorTramo(O.tiempoEnt, longAli) : 0;
  const dV = (montaje === "F.VIGA" || montaje === "F.TECHO") ? diasPorTramo(O.tiempoViga, longAli) : 0;
  ...
  add("Tubo alimentación (f.viga + pintado)", "ml", (montaje === "F.VIGA" ? r(fc.fviga * longAli) : 0), "ALB", T25);
  add("Tubo alimentación (f.techo + agujero + tapado + pintado 50x50cm)", "ud", (montaje === "F.TECHO" ? r(longAli / fc.ftechoDiv) : 0), "ALB", T25);
  ...
  return { diam, lineas: L, total: +total.toFixed(2) };
}
  ...
  app.get("/plan5/desglose", async function (req, res) {
    ...
    try {
      ...
      if (m && m.nsum && m.tipo && (m.longCon != null && m.longCon !== "")) {
        ...
        var diasRow = (fidx >= 0) ? med.rowOf["TUBO DE CONEXION|Fontanero (tubo conexión)|Tramo " + (fidx + 1) + " · días"] : null;
        var diasVal = (R.conexion && R.conexion.dias != null) ? R.conexion.dias : null;
