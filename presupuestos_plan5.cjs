...
// Build: 2026-06-14 v0.24 (TUBO DE ALIMENTACION ajustado al Excel/normativa + Toma de datos. MOTOR calcAlimentacion: (1) Tuerca reduccion 3' a ® AÑADIDA (faltaba): cant 1, salvo 0 si Ø90 (3', no reduce); precio del Sheet por pulgada; si sale Ø110 (4', sin precio) -> aviso. (2) Dias por diasAli con corte '<=' (6,5 m = tramo corto, como Excel L8) + TERCER TRAMO 15-20 m (font 2 / ent 4 / viga 1,5; saltamos el tope de 15 SOLO para dias, el diametro sigue la norma). (3) SOLO PIECERIA / tubo <2,5 m: fontanero fijo 0,50 dia, albañiles 0, PE 0 m. (4) f.viga decimal (sin redondeo, como Excel); f.techo redondeo HACIA ARRIBA (ceil). (5) avisos[] para material con cantidad>0 sin precio. TOMA DE DATOS: orden Montaje->Longitud->Codos; Longitud pasa a DESPLEGABLE 2,5/3/4...20; SOLO PIECERIA bloquea Longitud y Codos a 0. Validado: node --check + ejecucion (dias 6,5->1, 16-20->2, pieceria 0,5, f.viga/f.techo, Tuerca Ø63/90/110+aviso) + conexion IDENTICA (1.293 € con tope SP) + jsdom Toma de datos. PENDIENTE de esta tanda: MEDICIONES (Dato mas pequeño+derecha, igualar columnas, quitar columna Capitulo + tooltip en Concepto), edicion inline de dias de alimentacion + pasar dias/factores al Sheet, mostrar avisos de alimentacion en el banner.)
  ...
  app.get("/plan5/desglose", async function (req, res) {
    ...
    try {
      ...
      if (m && m.nsum && m.tipo && (m.longCon != null && m.longCon !== "")) {
        ...
        dsg = { lineas: lineas, diam: R.conexion ? R.conexion.diam : null, error: R.conexion ? R.conexion.error : false, longCon: (+m.longCon || 0), avisos: (R.conexion && R.conexion.avisos) ? R.conexion.avisos : [] };
      }
