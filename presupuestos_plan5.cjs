...
const _P5_REVISION = "Rev-19.15 24/06/2026";
...
function _p5paginaImagenes(R, meta, cuadro, saved){
  ...
  for (var k = 0; k < 11; k++){
    ...
    var inner = idq
      ? ("<img class=\"p5img\" data-src=\"/plan5/imagen?id=" + idq + "&n=" + nn + tkq + "\" style=\"max-width:100%;max-height:100%;object-fit:contain;display:none\" alt=\"\"><span class=\"p5ph\" style=\"color:#b6c2d2;font-size:10px\">" + nn + ".png</span>")
  ...
  return "<div class=\"sheet p5imgs\">"
    + "<style>"
    + ".p5imgs .p5padron{width:46%;border-collapse:collapse;margin:6px 0 12px;font-size:11px;table-layout:fixed}"
    + ".p5imgs .p5padron th{background:#1f3a8a;color:#fff;padding:4px 6px;text-align:center}"
    + ".p5imgs .p5padron td.cc{text-align:center}"
    + ".p5imgs .p5padron td{border:1px solid #cbd5e1;padding:3px 6px}"
    + ".p5imgs .p5padron td.pl{font-weight:700;text-align:left;word-break:break-word}"
    + ".p5imgs .p5grid{display:grid;grid-template-columns:repeat(3,1fr);gap:8px}"
    + ".p5imgs .p5imgcell{display:flex;flex-direction:column;align-items:center}"
    + ".p5imgs .p5imgbox{width:100%;height:150px;border:1px solid #cbd5e1;border-radius:6px;display:flex;align-items:center;justify-content:center;overflow:hidden;background:#fff}"
    + ".p5imgs .p5imglab{font-size:10px;font-weight:700;color:#1f3a8a;text-transform:uppercase;margin-top:3px;text-align:center}"
    + "</style>"
    + "<table class=\"p5padron\"><colgroup><col style=\"width:48%\"><col style=\"width:26%\"><col style=\"width:26%\"></colgroup><thead><tr><th>" + (_p5esc(cab) || "CATASTRO") + "</th><th>USO</th><th>SUP. m&#178;</th></tr></thead><tbody>" + filas + "</tbody></table>"
    + "<div class=\"p5grid\">" + celdas + "</div>"
    + "</div>";
}
...
<head>
<meta charset="utf-8">
<title>Presupuesto Nº ${_p5esc(np)} - ${_p5esc(via)}${(num!=null&&num!=="")?(" "+_p5esc(String(num))):""} (${_p5esc(String(rev).split(" ")[0])}) (presupuesto)(FALTA FIRMA)</title>
<style>
  :root{ --navy:#004079; }
...
<div class="p5toolbar">
  ...
  <button class="p5btn" onclick="p5imprimir()">&#128424; Imprimir / Guardar PDF</button><script>function p5imprimir(){var b=document.querySelector('.p5btn');if(b&&b.getAttribute('data-busy')){return;}var L=[].slice.call(document.querySelectorAll('.p5img'));var pend=L.filter(function(im){return im.getAttribute('data-src');});if(!pend.length){window.print();return;}if(b){b.setAttribute('data-busy','1');b.disabled=true;}var total=pend.length,hechas=0;function upd(){if(b)b.textContent='Cargando fotos... '+hechas+'/'+total;}upd();function fin(){if(b){b.disabled=false;b.innerHTML='&#128424; Imprimir / Guardar PDF';b.removeAttribute('data-busy');}requestAnimationFrame(function(){requestAnimationFrame(function(){setTimeout(function(){window.print();},300);});});}function done(){hechas++;upd();if(hechas>=total){fin();}}pend.forEach(function(im){var url=im.getAttribute('data-src');fetch(url).then(function(r){if(!r.ok)throw 0;return r.blob();}).then(function(bl){return new Promise(function(res,rej){var fr=new FileReader();fr.onload=function(){res(fr.result);};fr.onerror=rej;fr.readAsDataURL(bl);});}).then(function(durl){im.onload=function(){im.style.display='';var ph=im.parentNode.querySelector('.p5ph');if(ph)ph.style.display='none';im.removeAttribute('data-src');done();};im.onerror=function(){im.removeAttribute('data-src');done();};im.src=durl;}).catch(function(){im.removeAttribute('data-src');done();});});}</script>
</div>
