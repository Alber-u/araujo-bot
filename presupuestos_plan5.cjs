...
const _P5_REVISION = "Rev-19.59 26/06/2026";
...
function _p5paginaImagenes(R, meta, cuadro, saved){
  ...
  return "<div class=\"sheet p5imgs\">"
    ...
    + "</div>"
    + "</div>"
    + "<script>(function(){function fit(){var bx=document.querySelector('.p5padbox');var t=bx&&bx.querySelector('.p5padron');if(!bx||!t)return;t.style.transform='';t.style.fontSize='';var ah=bx.clientHeight,fs=9;for(var k=0;k<4;k++){var nh=t.scrollHeight;if(nh<=ah||nh<=0)break;fs=fs*ah/nh;if(fs<3){fs=3;t.style.fontSize=fs+'px';break;}t.style.fontSize=fs+'px';}var n2=t.scrollHeight;if(n2>ah&&n2>0){t.style.transform='scaleY('+(ah/n2)+')';}}if(document.readyState!=='loading')fit();else document.addEventListener('DOMContentLoaded',fit);window.addEventListener('load',fit);})();</script>";
}
