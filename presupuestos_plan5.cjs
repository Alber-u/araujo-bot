...
const _P5_REVISION = "Rev-19.2 24/06/2026";
...
module.exports = function (app) {
  ...
  function _p5catListaRC(xml){
    xml=String(xml||""); var out=[];
    var blocks=xml.match(/<rcdnp>[\s\S]*?<\/rcdnp>/g);
    if(blocks){ for(var i=0;i<blocks.length;i++){ var rc=_p5catTag(blocks[i],"pc1")+_p5catTag(blocks[i],"pc2"); if(rc) out.push(rc); } }
    if(!out.length){ var b=/<bi>[\s\S]*?<\/bi>/.exec(xml); if(b){ var rc1=_p5catTag(b[0],"pc1")+_p5catTag(b[0],"pc2"); if(rc1) out.push(rc1); } }
    return out;
  }
  ...
  app.get("/plan5/catastro", async function (req, res) {
    ...
    try {
      ...
      var locUrl = base + "/Consulta_DNPLOC?Provincia=" + encodeURIComponent(prov) + "&Municipio=" + encodeURIComponent(muni) + "&Sigla=" + encodeURIComponent(sig) + "&Calle=" + encodeURIComponent(via) + "&Numero=" + encodeURIComponent(num) + "&Bloque=&Escalera=&Planta=&Puerta=";
      var locXml = await _p5catFetch(locUrl);
      var ferr = _p5catError(locXml);
      if (ferr) return res.json({ ok: false, error: "Catastro: " + ferr });
      var rcs = _p5catListaRC(locXml);
      if (!rcs.length) return res.json({ ok: false, error: "El Catastro no devolvio inmuebles para esa direccion. Revisa la calle y el numero." });
      if (rcs.length > 60) rcs = rcs.slice(0, 60);
      var filas = [];
      for (var i = 0; i < rcs.length; i++) {
        try {
          var rcUrl = base + "/Consulta_DNPRC?Provincia=" + encodeURIComponent(prov) + "&Municipio=" + encodeURIComponent(muni) + "&RC=" + encodeURIComponent(rcs[i]);
          var rcXml = await _p5catFetch(rcUrl);
          filas.push(_p5catInmueble(rcXml));
        } catch (e2) { /* un inmueble que falle no debe tumbar la lista */ }
      }
      filas = filas.filter(function (f) { return f && (f.planta || f.puerta || f.uso || f.sup); });
      res.json({ ok: true, filas: filas });
    } catch (e) {
