...
function ppS(t, h, n) {
  var M = { "SIMPLE":[1,0,0], "SIMPLE+1":[1,1,0], "1-SIMPLE":[1,0,h], "1-SIMPLE+1":[1,1,h],
    "SIMPLE-1":[1,0,h*(n+1)], "SIMPLE-2":[1,0,h*(2*n+1)], "1-SIMPLE-1":[1,0,h*(n+1)+h], "1-SIMPLE-2":[1,0,h*(2*n+1)+h],
    "DOBLE":[2,0,0], "DOBLE+1":[2,1,0], "DOBLE+2":[2,2,0], "1-DOBLE":[2,0,h], "2-DOBLE":[2,0,2*h],
    "1-DOBLE+1":[2,1,h], "2-DOBLE+1":[2,1,2*h], "1-DOBLE+2":[2,2,h], "DOBLE-1":[2,0,h], "DOBLE-2":[2,0,2*h], "2-DOBLE+2":[2,2,2*h] };
  return M[t] || [1,0,0];
}
function pTuboS(t, h, n) { var a = ppS(t, h, n); return a[0]*h*(n+1)*(n+2)/2 + a[1]*h*(n+2) - a[2]; }
function mPeineS(t, h, n) { var p = /\+2$/.test(t) ? 2 : (/\+1$/.test(t) ? 1 : 0); return (n + 1 + p) * h; }
// viviendas: base k·(n+1)+p, menos el prefijo (1-/2-) y el sufijo negativo (-1/-2). (pViv del HTML no restaba.)
function vivS(t, n) {
  var k = /DOBLE/.test(t) ? 2 : 1;
  var p = /\+2$/.test(t) ? 2 : (/\+1$/.test(t) ? 1 : 0);
  var pre = /^2-/.test(t) ? 2 : (/^1-/.test(t) ? 1 : 0);
  var neg = /-2$/.test(t) ? 2 : (/-1$/.test(t) ? 1 : 0);
  return k*(n+1) + p - pre - neg;
}
...
function calcMontantes(R, precios, obra) {
  ...
  add("PEINE H (f.viga + pintado)", "ml", (pe["F.VIGA"]||0) * fviga, "ALB", T25);
  ...
  for (var i = 1; i <= 8; i++) add("Fontanero (PEINE V-EXT -" + i + ")", "cuadrilla x2", dv[i-1] || 0, "MO", MO);
  // aislamiento + chapa
