...
const _P5_REVISION = "Rev-19.12 24/06/2026";
...
function _p5paginaImagenes(R, meta, cuadro, saved){
  ...
  var filas = cat.map(function(r){
    return "<tr><td class=\"pl\">Pl:" + _p5esc(r.planta||"") + " Pt:" + _p5esc(r.puerta||"") + "</td><td>" + _p5esc(r.uso||"") + "</td><td class=\"r\">" + _p5esc(r.sup||"") + "</td></tr>";
  }).join("");
  ...
  for (var k = 0; k < 11; k++){
    ...
    var inner = idq
      ? ("<img class=\"p5img\" src=\"/plan5/imagen?id=" + idq + "&n=" + nn + tkq + "\" style=\"max-width:100%;max-height:100%;object-fit:contain\" onerror=\"this.style.display='none';var p=this.parentNode.querySelector('.p5ph');if(p)p.style.display='block';\"><span class=\"p5ph\" style=\"display:none;color:#b6c2d2;font-size:10px\">" + nn + ".png</span>")
  ...
  return "<div class=\"sheet p5imgs\">"
    + "<div class=\"s2head\"><div class=\"s2sumi\">Presupuesto nº <b>" + _p5esc((meta&&meta.nPresupuesto)||"") + "</b></div><div class=\"s2tit\">Padrón e imágenes</div></div>"
    + "<style>"
    + ".p5imgs .p5padron{width:100%;border-collapse:collapse;margin:6px 0 12px;font-size:11px}"
    + ".p5imgs .p5padron th{background:#1f3a8a;color:#fff;padding:4px 6px;text-align:left}"
    + ".p5imgs .p5padron th.r,.p5imgs .p5padron td.r{text-align:right}"
    + ".p5imgs .p5padron td{border:1px solid #cbd5e1;padding:3px 6px}"
    + ".p5imgs .p5padron td.pl{font-weight:700}"
    ...
    + "</style>"
    + "<table class=\"p5padron\"><thead><tr><th>" + (_p5esc(cab) || "CATASTRO") + "</th><th>USO</th><th class=\"r\">SUP. m&#178;</th></tr></thead><tbody>" + filas + "</tbody></table>"
...
<div class="p5toolbar">
  ...
  <button class="p5btn" onclick="p5imprimir()">&#128424; Imprimir / Guardar PDF</button><script>function p5imprimir(){var L=[].slice.call(document.querySelectorAll('.p5img'));var b=document.querySelector('.p5btn');if(b){b.disabled=true;b.textContent='Cargando fotos...';}function uno(im){if(im.decode){return im.decode().catch(function(){});}if(im.complete){return Promise.resolve();}return new Promise(function(res){im.addEventListener('load',res,{once:true});im.addEventListener('error',res,{once:true});});}var ready=Promise.all(L.map(uno));var tope=new Promise(function(res){setTimeout(res,30000);});Promise.race([ready,tope]).then(function(){if(b){b.disabled=false;b.innerHTML='&#128424; Imprimir / Guardar PDF';}setTimeout(function(){window.print();},250);});}</script>
</div>
