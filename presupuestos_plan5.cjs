...
const _P5_REVISION = "Rev-19.74 27/06/2026";
...
function _p5paginaGrupoPresion(doc, comunidadTxt){
  ...
  // 1) marcar los placeholders conocidos con un centinela, 2) escapar el texto fijo,
  //    3) reinyectar el HTML de cada hueco. Asi el texto va escapado y las lineas son HTML.
  var html = String(doc.cuerpo).replace(/\[([a-z_]+)\]/gi, function(m, k){
  ...
  var parrafos = html.split(/\n\s*\n/).map(function(p){ return '<p class="legalp">'+p.replace(/\n/g,"<br>")+"</p>"; }).join("\n");
  return '<div class="sheet legal">\n  <div class="legalh">'+_p5esc(doc.titulo||"")+'</div>\n  '+parrafos+'\n</div>';
}
...
${ _p5paginaGrupoPresion(_docGP, _comunidadGP) }
  ...
  app.get(["/plan5/presupuesto", "/plan5/presupuesto/:nombre"], async function (req, res) {
    ...
    try {
      ...
      try {
        ...
        _docsGP = { mantener:(_man && _man.activo!=="0")?_man:null, renunciar:(_ren && _ren.activo!=="0")?_ren:null };
