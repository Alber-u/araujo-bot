...
(function(){
  ...
  if(list){ list.querySelectorAll('[data-p5act]').forEach(function(el){ el.addEventListener('click',function(){ var a=el.getAttribute('data-p5act'); if((a==='IMPORTAR CATASTRO'||a==='IMPORTAR IMAGENES')&&window.__PLAN5_SAVED__&&window.__PLAN5_SAVED__.estado==='cerrado'){alert('Presupuesto cerrado: abrelo con el candado para poder importar.');return;} if(a==='IMPRIMIR PRESUPUESTO'){ window.open('/plan5/presupuesto'+q(),'_blank'); list.hidden=true; } if(a==='IMPRIMIR DATOS'){ alert('IMPRIMIR DATOS: en construccion (pronto: toma de datos + materiales + tareas).'); list.hidden=true; } if(a==='IMPORTAR CATASTRO'){ list.hidden=true; (function(){ var g=function(id){var e=document.getElementById(id);return e?(e.value||''):'';}; var pob=g('f_poblacion'), cp=g('f_cp'); var _dir0=String(g('f_direccion')); var _dl=_dir0.toLowerCase(), _cut=_dir0.length; [',',';',' bloque',' portal',' escalera'].forEach(function(_m){var _p=_dl.indexOf(_m);if(_p>=0&&_p<_cut)_cut=_p;}); var _base=_dir0.slice(0,_cut).trim(); var num=((_base.match(/([0-9]+[A-Za-z]?) *$/)||[])[1])||''; var calle=(_base.replace(/ *[0-9]+[A-Za-z]? *$/,'').trim())||_base; function doImp(tv,nv){ var p=[]; p.push('tipovia='+encodeURIComponent(tv||g('f_tipovia'))); p.push('calle='+encodeURIComponent(nv||calle)); p.push('numero='+encodeURIComponent(num)); p.push('poblacion='+encodeURIComponent(pob)); p.push('cp='+encodeURIComponent(cp)); if(tk)p.push('token='+encodeURIComponent(tk)); fetch('/plan5/catastro?'+p.join('&')).then(function(r){return r.json();}).then(function(j){ if(window.catImport)window.catImport(j); else alert('No se pudo cargar la lista.'); }).catch(function(e){ alert('Error consultando el Catastro: '+e.message); }); } function refImport(){ var _r=prompt('Referencia catastral (la que aparece como PARCELA CATASTRAL, 14 caracteres, p. ej. 0037010TG4403N):'); if(_r==null)return; var _m=String(_r).toUpperCase().match(/[0-9A-Z]{14,20}/); var _rc=_m?_m[0].slice(0,14):''; if(_rc.length<14){ alert('Referencia no valida: deben ser al menos 14 letras o numeros.'); return; } var _p=[]; _p.push('rc='+encodeURIComponent(_rc)); _p.push('poblacion='+encodeURIComponent(pob)); _p.push('cp='+encodeURIComponent(cp)); if(tk)_p.push('token='+encodeURIComponent(tk)); fetch('/plan5/catastro/porref?'+_p.join('&')).then(function(r){return r.json();}).then(function(j){ if(window.catImport)window.catImport(j); else alert('No se pudo cargar la lista.'); }).catch(function(e){ alert('Error consultando el Catastro: '+e.message); }); } function pick(vias,cb){ var ov=document.createElement('div'); ov.style.cssText='position:fixed;inset:0;background:rgba(0,0,0,.5);z-index:99999;display:flex;align-items:center;justify-content:center'; var bx=document.createElement('div'); bx.style.cssText='background:#fff;max-width:420px;width:90%;max-height:70vh;overflow:auto;border-radius:10px;padding:14px'; var ti=document.createElement('div'); ti.textContent='Elige la calle en el Catastro:'; ti.style.cssText='font-weight:700;margin-bottom:10px;color:#0b2b4a'; bx.appendChild(ti); var _rb=document.createElement('button'); _rb.type='button'; _rb.textContent='🔎 Meter referencia catastral'; _rb.style.cssText='display:block;width:100%;text-align:left;padding:9px 10px;margin:4px 0;border:1px solid #0b2b4a;border-radius:8px;background:#e8f0fe;cursor:pointer;font-size:14px;font-weight:700;color:#0b2b4a'; _rb.onclick=function(){ if(ov.parentNode)document.body.removeChild(ov); refImport(); }; bx.appendChild(_rb); if(!vias.length){ var _nn=document.createElement('div'); _nn.textContent='(No se encontraron calles. Usa la referencia catastral.)'; _nn.style.cssText='font-size:12px;color:#64748b;margin:4px 0 2px'; bx.appendChild(_nn); } vias.forEach(function(v){ var b=document.createElement('button'); b.type='button'; b.textContent=(v.tv?(v.tv+' '):'')+v.nv+(num?(', '+num):''); b.style.cssText='display:block;width:100%;text-align:left;padding:9px 10px;margin:4px 0;border:1px solid #cbd5e1;border-radius:8px;background:#f8fafc;cursor:pointer;font-size:14px'; b.onclick=function(){ if(ov.parentNode)document.body.removeChild(ov); cb(v); }; bx.appendChild(b); }); var cc=document.createElement('button'); cc.type='button'; cc.textContent='Cancelar'; cc.style.cssText='display:block;width:100%;padding:9px;margin-top:8px;border:0;border-radius:8px;background:#e2e8f0;cursor:pointer'; cc.onclick=function(){ if(ov.parentNode)document.body.removeChild(ov); }; bx.appendChild(cc); ov.appendChild(bx); ov.onclick=function(e){ if(e.target===ov && ov.parentNode)document.body.removeChild(ov); }; document.body.appendChild(ov); } var vp=[]; vp.push('q='+encodeURIComponent(calle)); vp.push('poblacion='+encodeURIComponent(pob)); vp.push('cp='+encodeURIComponent(cp)); if(tk)vp.push('token='+encodeURIComponent(tk)); fetch('/plan5/catastro/vias?'+vp.join('&')).then(function(r){return r.json();}).then(function(j){ var vias=(j&&j.vias)||[]; pick(vias,function(v){ doImp(v.tv,v.nv); }); }).catch(function(){ pick([],function(v){ doImp(v.tv,v.nv); }); }); })(); } }); }); }
})();
`;
...
const CAPS_PRESUPUESTO = [
  ...
  "1.3.1.2 Batería de contadores", "1.3.2 Juego de llaves de escuadra",
  ...
  "2.1 Armario batería", "2.2 Punto de luz", "2.3 Sumidero de agua",
  "2.4 Regolas y taladros", "2.5 Falsos techos/vigas",
  "3.1 Grupo de presión", "3.2 Tubería de alimentación",
  "3.3 By-pass + llaves + v.antiretorno + p.material", "3.4 Depósito",
  "4.1 Forrado de montantes", "4.2 Canaleta de protección",
];
...
function parseMediciones(values, ovDato) {
  ...
  obra.montantes = {
    ...
    dEXT:   fb(gm("Fontanero (ENGANCHE - exterior)", "Días por vivienda"), 1),
    dFac:   fb(gm("Fontanero (ENGANCHE - interior fácil)", "Días por vivienda"), 1.5),
...
function paso3_subvencion(R, F) {
  ...
  const viv = (R.entrada.nsum || 0) - (R.entrada.puntosComunidad ? 1 : 0);  // F22 = viviendas + locales (= nsum - comunidad), como el Excel
...
function paso6_emasesaNeto(R, F) {
  ...
  const em = calcEmasesa({
    ...
    viviendas:       (R.entrada.nsum || 0) - (R.entrada.puntosComunidad ? 1 : 0),   // F22 = viviendas + locales (subvención/contadores/comunero, como el Excel)
...
function _p5tablaPresupuesto(dsg, cuadro, matX){
  var _matConexX=(matX&&matX.conex)||"", _matAlimX=(matX&&matX.alim)||"", _diamGPX=(matX&&matX.diamGP)||"", _diamDepX=(matX&&matX.diamDep)||"", _otrosEspX=(matX&&matX.otrosEsp)||"";
  var _matCuartoX=(function(){ var t=String((matX&&matX.cuarto)||"").toUpperCase(); if(t==="ALUMINIO")return "aluminio, puertas aluminio"; if(t==="OBRA - P.ALUMINIO")return "obra, puertas aluminio"; if(t==="OBRA - P.HIERRO")return "obra, puertas hierro"; if(t==="EXISTENTE")return "Cuarto existente"; return ""; })();
  ...
  function celdasLinea(r){
    ...
    } else if(r.fuentes.length===1 && String(r.num).indexOf("3.")===0){
      var l3=r.fuentes[0];
      precio = _p5eur((l3.precio||0)*fac(l3.tipo));
      cant = _p5num(l3.cantidad) + (l3.ud?(" "+_p5esc(l3.ud)):"");
    ...
    else if(r.num==="1.3.2") mat="entrada y salida";
    else if(r.num==="2.5") mat="Escayola";
    else if(r.num==="2.1") mat=_p5esc(_matCuartoX);
    else if(r.num==="4.2") mat="Aluminio";
    else if(r.num==="4.1") mat="Coquilla";
    var _gpCombo = (r.num==="3.1" && _diamGPX) ? _p5esc(_diamGPX) : ((r.num==="3.4" && _diamDepX) ? _p5esc(_diamDepX) : ((r.num==="2.6" && _otrosEspX) ? _p5esc(_otrosEspX) : ""));
...
// ============================ SISTEMA DE REVISIONES (x.y) ============================
// La version tiene forma x.y y se gobierna a dos manos:
//   * La X (mayor) la manda GUILLE. Solo sube cuando el considera que lo conseguido es lo
//     bastante importante (un hito). Cuando Guille pide subir la X, la Y se reinicia a 1.
//   * La Y (menor) la manda el MOTOR (Claude). El RESULTADO de este archivo es la IMPRESION del
//     presupuesto. REGLA: se compara la impresion que sale con la de la revision ACTUAL. Si un
//     cambio imprime IGUAL que la revision actual -> la Y NO sube. Si imprime DISTINTO (por
//     CALCULO, por FRASES/textos o por ESTETICA) -> la Y SUBE. Los cambios que no alteran la
//     impresion (toma de datos, mapa, arreglos internos, este comentario) no cuentan.
// Resumen: Guille manda en la X, el motor manda en la Y.
// Arranque oficial de este sistema: 20.1 (con 5 presupuestos ya enviados en real y todo probado).
// ====================================================================================
const _P5_REVISION = "Rev-22.2 24/09/2026";
...
const _P5_PIE_DISTRIB = "data:image/png;base64,";
const _P5_DIST_JS = `(function(){var D=window.__P5_DIST__;var cv=document.getElementById('p5distcv');if(!cv||!D)return;var ctx=cv.getContext('2d');var W=cv.width,H=cv.height;var z=W/600;var pad=10;var legAspect=D.legAspect||2.776;var legW=W-2*pad,legH=Math.round(legW/legAspect);var topH=H-legH-2*pad;ctx.clearRect(0,0,W,H);ctx.fillStyle='#fff';ctx.fillRect(0,0,W,H);function unpack(cr){if(!cr||!Array.isArray(cr.base))return cr;var bx=cr.base[0],by=cr.base[1];var U=function(pp){return [pp[0]+bx,pp[1]+by];};var Up=function(a){return (a||[]).map(U);};var Upoly=function(a){return (a||[]).map(Up);};var s2=(cr.strokes||[]).map(function(st){return {c:st.c,w:st.w,pts:Up(st.pts)};});var bg2=null;if(cr.bg){bg2={world:Up(cr.bg.world)};if(cr.bg.edificios)bg2.edificios=Upoly(cr.bg.edificios);if(cr.bg.patios)bg2.patios=Upoly(cr.bg.patios);}return {strokes:s2,bg:bg2};}var cru=unpack(D.croquis)||{};var strokes=cru.strokes||[],bg=cru.bg||null;var peines=D.peines||[],textos=D.textos||[],simbolos=D.simbolos||[],lineas=D.lineas||[];ctx.font='bold '+(44*z)+'px Arial';var E=[],any=false;function addE(x,y,l,r,t,b){x=+x;y=+y;if(!isFinite(x)||!isFinite(y))return;any=true;E.push({x:x,y:y,l:l,r:r,t:t,b:b});}if(bg){(bg.world||[]).forEach(function(p){addE(p[0],p[1],2,2,2,2);});(bg.edificios||[]).forEach(function(g){(g||[]).forEach(function(p){addE(p[0],p[1],1,1,1,1);});});(bg.patios||[]).forEach(function(g){(g||[]).forEach(function(p){addE(p[0],p[1],1,1,1,1);});});}if(!(bg&&bg.world&&bg.world.length>=3)){strokes.forEach(function(s){var hw=((s&&s.w)||3)/2;(s.pts||[]).forEach(function(p){addE(p[0],p[1],hw,hw,hw,hw);});});}lineas.forEach(function(l){var hw=7*z;addE(+l.x1,+l.y1,hw,hw,hw,hw);addE(+l.x2,+l.y2,hw,hw,hw,hw);});peines.forEach(function(pe){var R=21*z;addE(+pe.x,+pe.y,R,R,R,R);});textos.forEach(function(te){var w=ctx.measureText(String(te.t)).width;addE(+te.x,+te.y,3,w+3,26*z,26*z);});simbolos.forEach(function(sm){var kk=sm.k;if(kk==='portal'||kk==='gp'){var w=ctx.measureText(kk==='portal'?'PORTAL':'GP').width;addE(+sm.x,+sm.y,w/2+3,w/2+3,26*z,26*z);}else if(kk==='llave'){addE(+sm.x,+sm.y,21*z,21*z,18*z,18*z);}else{addE(+sm.x,+sm.y,21*z,21*z,21*z,21*z);}});var k=1,ox=0,oy=0;if(any){var GX0=Infinity,GX1=-Infinity,GY0=Infinity,GY1=-Infinity;E.forEach(function(e){if(e.x<GX0)GX0=e.x;if(e.x>GX1)GX1=e.x;if(e.y<GY0)GY0=e.y;if(e.y>GY1)GY1=e.y;});var ip=12,availW=W-2*ip,availH=topH-2*ip;k=Math.min(availW/((GX1-GX0)||1),availH/((GY1-GY0)||1));for(var it=0;it<16;it++){ox=(W-k*(GX0+GX1))/2;oy=(topH-k*(GY0+GY1))/2;var sx0=Infinity,sx1=-Infinity,sy0=Infinity,sy1=-Infinity;E.forEach(function(e){var px=e.x*k+ox,py=e.y*k+oy;if(px-e.l<sx0)sx0=px-e.l;if(px+e.r>sx1)sx1=px+e.r;if(py-e.t<sy0)sy0=py-e.t;if(py+e.b>sy1)sy1=py+e.b;});var curW=sx1-sx0,curH=sy1-sy0;if(curW<=availW+0.5&&curH<=availH+0.5)break;var f=Math.min(availW/curW,availH/curH);if(!(f>0)||f>=0.999)break;k=k*f*0.99;}ox=(W-k*(GX0+GX1))/2;oy=(topH-k*(GY0+GY1))/2;var mnx=Infinity,mxx=-Infinity,mny=Infinity,mxy=-Infinity;E.forEach(function(e){var px=e.x*k+ox,py=e.y*k+oy;if(px-e.l<mnx)mnx=px-e.l;if(px+e.r>mxx)mxx=px+e.r;if(py-e.t<mny)mny=py-e.t;if(py+e.b>mxy)mxy=py+e.b;});if(mnx<ip)ox+=(ip-mnx);else if(mxx>W-ip)ox+=(W-ip-mxx);if(mny<ip)oy+=(ip-mny);else if(mxy>topH-ip)oy+=(topH-ip-mxy);}var w2s=function(p){return [p[0]*k+ox,p[1]*k+oy];};if(any){if(bg&&bg.world&&bg.world.length>=3){ctx.save();ctx.beginPath();bg.world.forEach(function(p,ii){var qq=w2s(p);if(ii===0)ctx.moveTo(qq[0],qq[1]);else ctx.lineTo(qq[0],qq[1]);});ctx.closePath();ctx.fillStyle=(bg.edificios&&bg.edificios.length)?'#a8d8c5':'#f3c9c9';ctx.fill();ctx.lineWidth=2;ctx.strokeStyle='#334155';ctx.stroke();var pol=function(ps,fl,stc){if(!ps)return;ps.forEach(function(pg){if(!pg||pg.length<3)return;ctx.beginPath();pg.forEach(function(p,ii){var s=w2s(p);if(ii===0)ctx.moveTo(s[0],s[1]);else ctx.lineTo(s[0],s[1]);});ctx.closePath();ctx.fillStyle=fl;ctx.fill();ctx.lineWidth=1;ctx.strokeStyle=stc;ctx.stroke();});};pol(bg.edificios,'#f3c9c9','#b06a6a');pol(bg.patios,'#bfe0d3','#6a9c8a');ctx.restore();}strokes.forEach(function(st){if(!st||!st.pts||!st.pts.length)return;ctx.save();ctx.strokeStyle=st.c||'#111111';ctx.fillStyle=st.c||'#111111';ctx.lineWidth=(st.w||3);ctx.lineCap='round';ctx.lineJoin='round';if(st.pts.length===1){var qq=w2s(st.pts[0]);ctx.beginPath();ctx.arc(qq[0],qq[1],(st.w||3)/2,0,Math.PI*2);ctx.fill();}else{ctx.beginPath();st.pts.forEach(function(p,ii){var qq=w2s(p);if(ii===0)ctx.moveTo(qq[0],qq[1]);else ctx.lineTo(qq[0],qq[1]);});ctx.stroke();}ctx.restore();});lineas.forEach(function(l){var a=w2s([+l.x1,+l.y1]),b=w2s([+l.x2,+l.y2]);ctx.save();if(l.k==='alim'){ctx.strokeStyle='#16a34a';ctx.lineWidth=14*z;ctx.setLineDash([18*z,12*z]);}else if(l.k==='pzzcc'){ctx.strokeStyle='#2563eb';ctx.lineWidth=12*z;ctx.setLineDash([16*z,10*z]);}else{ctx.strokeStyle='#dc2626';ctx.lineWidth=12*z;ctx.setLineDash([18*z,12*z]);}ctx.lineCap='butt';ctx.beginPath();ctx.moveTo(a[0],a[1]);ctx.lineTo(b[0],b[1]);ctx.stroke();ctx.setLineDash([]);ctx.restore();});peines.forEach(function(pe){var s=w2s([+pe.x,+pe.y]);var R=21*z;ctx.save();ctx.beginPath();ctx.arc(s[0],s[1],R,0,Math.PI*2);ctx.fillStyle='#2563eb';ctx.fill();ctx.lineWidth=2*z;ctx.strokeStyle='#1e3a8a';ctx.stroke();ctx.fillStyle='#ffffff';ctx.font='bold '+(R*1.575)+'px Arial';ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText(String(pe.n),s[0],s[1]+1);ctx.restore();});textos.forEach(function(te){var s=w2s([+te.x,+te.y]);ctx.save();ctx.font='bold '+(44*z)+'px Arial';ctx.textAlign='left';ctx.textBaseline='middle';ctx.fillStyle='#dc2626';ctx.fillText(String(te.t),s[0],s[1]);ctx.restore();});simbolos.forEach(function(sm){var s=w2s([+sm.x,+sm.y]);var sx=s[0],sy=s[1],kk=sm.k;ctx.save();if(kk==='llave'){ctx.fillStyle='#16a34a';var tw=21*z,th=18*z;ctx.beginPath();ctx.moveTo(sx-tw,sy-th);ctx.lineTo(sx,sy);ctx.lineTo(sx-tw,sy+th);ctx.closePath();ctx.moveTo(sx+tw,sy-th);ctx.lineTo(sx,sy);ctx.lineTo(sx+tw,sy+th);ctx.closePath();ctx.fill();}else if(kk==='azotea'){ctx.beginPath();ctx.arc(sx,sy,21*z,0,Math.PI*2);ctx.fillStyle='#dc2626';ctx.fill();}else if(kk==='bateria'){ctx.fillStyle='#16a34a';ctx.fillRect(sx-21*z,sy-21*z,42*z,42*z);}else if(kk==='gp'){ctx.fillStyle='#16a34a';ctx.font='bold '+(44*z)+'px Arial';ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText('GP',sx,sy);}else if(kk==='portal'){ctx.fillStyle='#dc2626';ctx.font='bold '+(44*z)+'px Arial';ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText('PORTAL',sx,sy);}ctx.restore();});}if(D.legend){var img=new Image();img.onload=function(){try{ctx.drawImage(img,pad,H-legH-pad,legW,legH);}catch(e){}};img.src=D.legend;}})();`;
...
function _p5paginasLegales(meta, cuadro, condiciones){
  ...
  <p class="legalp">8.1. Este presupuesto tendrá una validez de 1 mes.</p>
  <p class="legalp">8.2. Tendrá la consideración de comunicación o requerimiento válido el remitido por LA EMPRESA por correo electrónico a la dirección que figura en la portada del presente presupuesto. Los plazos de esta cláusula se computarán desde el envío de la comunicación de la que traigan causa. La Comunidad responde del cumplimiento de sus comuneros, sin que la falta de colaboración de estos, cualquiera que sea su número, la exonere.</p>
...
function _p5paginaSubvencion(R, meta, cuadro){
  ...
  <div class="s2box">
    ...
    <table class="s2grid s2inst"><tbody>
      <tr><td>INSTALADOR: <b>Instalaciones Araujo (Ara Corporate Sdad. Inv. SL)</b></td><td class="r">Nº de viviendas y/o locales: <b>${((R.entrada&&R.entrada.nsum)||0)-((R.entrada&&R.entrada.puntosComunidad)?1:0)}</b></td></tr>
    </tbody></table>
...
// Logo oficial EMASESA (extraido del impreso original) para la pagina "Impreso para toma de datos".
var _P5_LOGO_EMASESA = "data:image/jpeg;base64,";
// Devuelve {dia,mes,anio} de la fecha del presupuesto (admite yyyy-mm-dd o dd/mm/yyyy); anio 4 digitos.
function _p5fechaImpreso(sv){
  var ss=String(sv||""); var d,mo,y;
  var m=/^(\d{4})-(\d{2})-(\d{2})/.exec(ss);
  if(m){ y=m[1]; mo=parseInt(m[2],10); d=parseInt(m[3],10); }
  else { var m2=/^(\d{1,2})[\/.](\d{1,2})[\/.](\d{4})/.exec(ss); if(m2){ d=parseInt(m2[1],10); mo=parseInt(m2[2],10); y=m2[3]; } }
  if(!y){ var dt=new Date(); d=dt.getDate(); mo=dt.getMonth()+1; y=String(dt.getFullYear()); }
  var M=["enero","febrero","marzo","abril","mayo","junio","julio","agosto","septiembre","octubre","noviembre","diciembre"];
  return { dia:String(d), mes:(M[mo-1]||""), anio:String(y) };
}
// Pagina: "Impreso para toma de datos" de EMASESA/Plan Cinco (replica del formulario oficial). Va tras las fotos.
function _p5paginaImpresoEmasesa(R, meta, saved){
  var f = (R && R.finca) || {};
  var sp = _p5splitDir((meta && meta.direccion) || f.direccion || "");
  ...
  var LOGO = _P5_LOGO_EMASESA;
  return `<div class="sheet p5emasesa">
  <style>
  .p5emasesa{font-family:Arial,Helvetica,sans-serif;color:#000;font-size:10.2px;line-height:1.32}
  .sheet.p5emasesa{display:flex;flex-direction:column}
  .p5emasesa .embox{border:1.3px solid #000}
  .p5emasesa .emhd{display:flex;align-items:stretch}
  .p5emasesa .emhd-logo{width:128px;border-right:1.3px solid #000;display:flex;align-items:center;justify-content:center;padding:3px}
  .p5emasesa .emhd-logo img{width:74px;height:auto;display:block}
  .p5emasesa .emhd-mid{flex:1;border-right:1.3px solid #000;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:6px 8px}
  .p5emasesa .emhd-mid .empc{font-family:"Segoe Script","Brush Script MT","Lucida Handwriting",cursive;font-style:italic;font-size:15px;color:#111;line-height:1;margin-bottom:3px}
  .p5emasesa .emhd-mid .emti{font-weight:bold;font-size:14px}
  .p5emasesa .emhd-sum{width:200px;display:flex;align-items:center;padding:6px 8px;font-size:10.5px}
  .p5emasesa .embar{border-top:1.3px solid #000;text-align:center;font-weight:bold;font-size:12px;padding:3px 4px}
  .p5emasesa .embar .emsub{font-weight:normal;color:#333;font-size:10px;background:#d9d9d9;padding:1px 5px;border-radius:1px}
  .p5emasesa .emsec{border-top:1.3px solid #000;padding:3px 9px 4px}
  .p5emasesa .emrow{display:flex;align-items:flex-end;margin:0;min-height:20px}
  .p5emasesa .emlbl{white-space:nowrap;line-height:1}
  .p5emasesa .emval{white-space:nowrap;padding:0 4px;font-weight:bold;line-height:1;text-transform:uppercase}
  .p5emasesa .emdots{flex:1;border-bottom:2px dotted #000;height:0;margin:0 4px;min-width:18px}
  .p5emasesa .emchk{display:flex;align-items:flex-start;gap:8px;margin:5px 0}
  .p5emasesa .emcb{width:15px;height:15px;border:1.3px solid #000;flex:0 0 auto;margin-top:1px}
  .p5emasesa .emnota{font-size:8.7px;line-height:1.28;text-align:justify;margin:5px 0}
  .p5emasesa .eminfo{margin:6px 0 2px}
  .p5emasesa .embullets div{display:flex;gap:8px;margin:4px 0;text-align:justify}
  .p5emasesa .emfirma-l{text-align:center;margin:8px 0 2px}.p5emasesa .emfirma-l b{text-transform:uppercase}
  .p5emasesa .emfirma-r{text-align:center;margin:10px 0 0}
  .p5emasesa .emlegal{font-style:italic;font-size:7.5px;line-height:1.28;color:#222;text-align:justify;margin-top:auto;padding-top:10px}
  .p5emasesa .emlegal p{margin:7px 0}
  .p5emasesa .emu{text-decoration:underline}
  </style>
  <div class="embox">
    <div class="emhd">
      <div class="emhd-logo"><img src="${LOGO}" alt="EMASESA"></div>
      <div class="emhd-mid"><div class="empc">Plan Cinco</div><div class="emti">Impreso para toma de datos</div></div>
      <div class="emhd-sum">Nº de Suministro colectivo:</div>
    </div>
    <div class="embar">Solicitante del contrato de Suministro <span class="emsub">(Rellenar en todos los casos)</span></div>
    <div class="emsec" style="border-top:none">
      <div class="emrow"><span class="emlbl">Nombre y Apellidos:</span><span class="emval">${_p5esc(nombre)}</span><span class="emdots"></span><span class="emlbl">NIF/CIF:</span><span class="emdots" style="flex:0 0 150px"></span></div>
      <div class="emrow"><span class="emlbl">Dirección:</span><span class="emval">${_p5esc(dirCalle)}</span><span class="emdots"></span><span class="emlbl">Nº:</span><span class="emval">${_p5esc(num)}</span><span class="emdots" style="flex:0 0 150px"></span></div>
      <div class="emrow"><span class="emlbl">Piso:</span><span class="emdots" style="flex:0 0 70px"></span><span class="emlbl">Población:</span><span class="emval">${_p5esc(poblacion)}</span><span class="emdots"></span><span class="emlbl">Teléfono (fijo):</span><span class="emdots"></span><span class="emlbl">Móvil:</span><span class="emdots" style="flex:0 0 110px"></span></div>
      <div class="emrow"><span class="emlbl">Correo electrónico:</span><span class="emdots"></span><span class="emlbl">Nº habitantes:</span><span class="emdots" style="flex:0 0 150px"></span></div>
      <div class="eminfo">Habiendo sido informado de que actualmente EMASESA factura en función del número de habitantes:</div>
      <div class="emchk"><span class="emcb"></span><span><b>Entrega Certificado de empadronamiento.</b></span></div>
      <div class="emchk"><span class="emcb" style="display:flex;align-items:center;justify-content:center;font-weight:bold;font-size:13px;line-height:1">✕</span><span><b>No entrega Certificado de empadronamiento.</b> La facturación de consumo se realizará teniendo en cuenta <b class="emu">0 hab.</b></span></div>
      <div class="emnota"><b class="emu">NOTA IMPORTANTE</b>: El art. 2.1 de la Ordenanza Reguladora del Servicio dispone que <i>"El nº de habitantes por vivienda deberá ser acreditado mediante inscripción del titular del suministro en el Padrón Municipal del Ayuntamiento correspondiente. Las modificaciones en más o en menos del nº de habitantes de la vivienda deberán ser notificadas a EMASESA por la persona usuaria en el momento en que se produzcan, <u>siendo responsabilidad del mismo tener actualizado los datos en el Padrón Municipal y en Emasesa</u>. La notificación a EMASESA de la actualización de los datos del nº de habitantes […] no tendrá efectos retroactivos, por lo que comenzará a generar efectos a partir de la primera factura que se emita con posterioridad a la presentación de la solicitud, acompañada de la documentación acreditativa del cumplimiento de los requisitos.</i></div>
      <div class="emchk"><span class="emcb"></span><span>El solicitante autoriza a Emasesa para que las facturas y comunicaciones ordinarias le sean remitidas a través del correo electrónico que figura en este documento y en el contrato que se formalizará como consecuencia de la individualización.</span></div>
    </div>
    <div class="embar">Datos de contacto <span class="emsub">(Rellenar sólo en caso de no residir en la finca del suministro)</span></div>
    <div class="emsec" style="border-top:none">
      <div class="emrow"><span class="emlbl">Nombre y apellidos:</span><span class="emdots"></span><span class="emlbl">NIF/CIF:</span><span class="emdots" style="flex:0 0 150px"></span></div>
      <div class="emrow"><span class="emlbl">Dirección:</span><span class="emdots"></span><span class="emlbl">Nº:</span><span class="emdots" style="flex:0 0 150px"></span></div>
      <div class="emrow"><span class="emlbl">Población:</span><span class="emdots"></span><span class="emlbl">Provincia:</span><span class="emdots" style="flex:0 0 200px"></span></div>
      <div class="emrow"><span class="emlbl">C.P.:</span><span class="emdots" style="flex:0 0 90px"></span><span class="emlbl">Teléfono fijo:</span><span class="emdots"></span><span class="emlbl">Móvil:</span><span class="emdots" style="flex:0 0 150px"></span></div>
    </div>
    <div class="embar">Datos Bancarios <span class="emsub">(Rellenar si desea que EMASESA cobre las facturas a través de su cuenta bancaria)</span></div>
    <div class="emsec" style="border-top:none">
      <div class="emrow"><span class="emlbl">Titular de la C/C o Libreta:</span><span class="emdots"></span></div>
      <div class="emrow"><span class="emlbl">Entidad Bancaria:</span><span class="emdots"></span></div>
      <div class="emrow" style="margin-top:9px"><span class="emlbl">Nº de IBAN (24 dígitos):</span><span class="emdots"></span></div>
    </div>
  </div>
  <div class="eminfo" style="margin-top:12px">El solicitante del contrato de suministro:</div>
  <div class="embullets">
    <div><span>-</span><span>Queda informado de que el ingreso que tiene que realizar (obra/contratación), se hará exclusivamente en la cuenta que Emasesa tiene habilitada para tal efecto, y con el documento de pago que se le entregará.</span></div>
    <div><span>-</span><span>Expresa su acuerdo en la instalación de batería de contadores individuales y declara que los datos reflejados en el presente impreso son ciertos, y para que así conste, firmo la presente en</span></div>
  </div>
  <div class="emfirma-l">Sevilla, a&nbsp;&nbsp; <b>${_p5esc(fp.dia)}</b> &nbsp;de&nbsp; <b>${_p5esc(fp.mes)}</b> &nbsp;de&nbsp; <b>${_p5esc(fp.anio)}</b></div>
  <div class="emfirma-r">Firma del presidente</div>
  <div class="emlegal">
    <p>En virtud del nuevo Reglamento (UE) General Europeo de Protección de Datos 2016/679 le informamos que la EMPRESA METROPOLITANA DE ABASTECIMIENTO Y SANEAMIENTO DE AGUAS DE SEVILLA, S.A. (EMASESA), con NIF A-41039496, y domicilio social en calle Escuelas Pías 1, 41003 Sevilla, es la Responsable del tratamiento de datos de carácter personal. Para nosotros es muy importante que como usuario de EMASESA entienda perfectamente qué vamos a hacer con su información personal. Para ello le daremos el control de sus datos, con un lenguaje sencillo y opciones claras que permitan decidir qué haremos con su propia información.</p>
    <p>Necesitamos sus datos para poder atender sus solicitudes y prestarle nuestros servicios; sólo tendrá conocimiento de su información el personal de EMASESA que esté debidamente autorizado y aquellos terceros que lo requieran pero siempre informando previamente de este aspecto o pidiendo su consentimiento. Tiene derecho a saber de qué información disponemos, corregirla y eliminarla, tal y como se explica en la información adicional disponible en nuestra página web (http://www.emasesa.com/politica-de-privacidad/).</p>
  </div>
</div>`;
}
...
// Pagina: DOCUMENTACION (de la Comunidad). Va justo despues de la pagina de las fotos.
function _p5paginaDocumentacion(R, meta, cuadro, saved){
  var C = cuadro || {}; var em = C.emasesa || {};
  var np = _p5esc((meta && meta.nPresupuesto) || "");
  var fecha = _p5esc(_p5fechaLarga((meta && meta.fecha) || ""));
  var total = (C.totP5Iva!=null) ? _p5eur(C.totP5Iva) : "__________ €";
  var pc = (em.porComunero!=null) ? em.porComunero : (C.comunero!=null ? C.comunero : null);
  var porCom = (pc!=null) ? _p5eur(pc) : "__________ €";
  return "<div class='sheet'>"
    + "<style>"
    + ".p5doc ol{margin:0;padding-left:22px}"
    + ".p5doc li{margin:0 0 22px;font-size:11pt;line-height:1.4;text-align:justify}"
    + ".p5doc .nota{font-style:italic;color:#888}"
    + ".p5doc .notaint{font-style:italic;color:#888;text-decoration:underline;font-size:11pt;line-height:1.4;text-align:justify;margin:0 0 22px 22px}"
    + ".p5doc li::marker{font-weight:700}"
    + ".p5doc .acta{background:#f3f4f6;padding:10px 14px;margin:6px 0 2px;font-style:italic;color:#888;font-size:11pt;line-height:1.45;text-align:justify}"
    + ".p5doc .acta p{margin:0 0 22px}.p5doc .acta p:last-child{margin:0}"
    + "</style>"
    + "<div class='sech'>9.- DOCUMENTACIÓN (de la Comunidad)</div>"
    + "<div class='p5doc'>"
    + "<ol>"
    + "<li>Presupuesto firmado por el Presidente/a <span class='nota'>(EMASESA no admite la firma del Administrador/a)</span>.</li>"
    + "<li>Listado completo de Piso-Vecino-WhatsApp <span class='nota'>(para gestionar la documentación)</span>.</li>"
    + "</ol>"
    + "<div class='notaint'>(Prioritario: envíennos cuanto antes estos dos primeros requisitos para arrancar su Expediente; el resto de documentos pueden aportarlos más adelante)</div>"
    + "<ol start='3'>"
    + "<li>Solicitud para EMASESA a nombre de la CCPP <span class='nota'>(documento adjunto)</span>.</li>"
    + "<li>Copia del NIF de la CCPP.</li>"
    + "<li>Acta de nombramiento del Presidente/a.</li>"
    + "<li>Aprobación del Presupuesto en Acta. <span class='nota'>(Redacción sugerida para el Acta)</span>"
    + "<div class='acta'>"
    + "<p>“- La CCPP aprueba (por unanimidad o por las 3/5 partes) la realización de las obras necesarias para el cambio de sistema de abastecimiento de agua, sustituyendo el contador comunitario por contadores individuales.</p>"
    + "<p>- La CCPP aprueba igualmente el presupuesto nº " + (np || "__________") + " de fecha " + (fecha || "__________") + " de la Empresa Instaladora ARA Corporate Sociedad de Inversión S.L. (en adelante LA EMPRESA) por un importe de " + total + " IVA Incluido sin descontar subvención de EMASESA, lo que una vez descontada le supondrá a pagar a cada propietario un importe de " + porCom + ".</p>"
    + "<p>- La CCPP queda expresamente enterada de que el ingreso que tenga que realizar cada vecino (obra/contratación) se hará exclusivamente en la cuenta que EMASESA tiene habilitada para tal efecto y con el documento de pago que se le entregará.</p>"
    + "<p>- La CCPP pone en conocimiento de LA EMPRESA y de EMASESA para que proceda a la instalación de montantes a aquellos propietarios que faciliten la documentación necesaria y abonen el importe de la obra correspondiente.”</p>"
    + "</div>"
    + "</li>"
    + "<li>Impreso de mantenimiento/renuncia a grupo de presión <span class='nota'>(documento adjunto)</span>.</li>"
    + "<li>Última factura de EMASESA.</li>"
    + "<li>Autorización de paso de montantes <span class='nota'>(documento adjunto si procede)</span>.</li>"
    + "</ol>"
    + "</div>"
    + "</div>";
}
...
function _p5paginaImagenes(R, meta, cuadro, saved){
  var f = (R && R.finca) || {};
  var _padFS = 9.25;   // <-- TAMANO DE LETRA del padron (px). Subir/bajar a mano hasta que quede bien.
  var _padLH = 0.85;  // <-- interlineado del padron (altura de fila). Subir/bajar a mano.
  var _padWdir = 52;  // <-- ancho columna DIRECCION (%)
  var _padWuso = 20;  // <-- ancho columna USO (%)
  var _padWsup = 14;  // <-- ancho columna SUP. m2 (%)
  var _padWbat = 14;  // <-- ancho columna BAT (%)   (los cuatro deben sumar 100)
  var _padFilas = 62;  // <-- nº de filas que se dibuja SIEMPRE (si hay menos pisos, rellena en blanco)
  var sv = saved || {};
  ...
  var _filasArr = cat.map(function(r){
    return "<tr><td class=\"pl\">Pl:" + _p5esc(r.planta||"") + " Pt:" + _p5esc(r.puerta||"") + "</td><td class=\"cc\">" + _p5esc(r.uso||"") + "</td><td class=\"cc\">" + _p5esc(r.sup||"") + "</td><td class=\"cc\">" + _p5esc(r.bat||"") + "</td></tr>";
  });
  for (var _fi = _filasArr.length; _fi < _padFilas; _fi++){ _filasArr.push("<tr><td class=\"pl\">&nbsp;</td><td class=\"cc\"></td><td class=\"cc\"></td><td class=\"cc\"></td></tr>"); }
  var filas = _filasArr.join("");
  ...
  var celdas = "";
  var _sv = saved || {};
  var _cq = _sv.croquis || null;
  var _hasCroq = !!((_cq && ((_cq.strokes && _cq.strokes.length) || (_cq.bg && _cq.bg.world && _cq.bg.world.length))) || (_sv.croquisPeines && _sv.croquisPeines.length) || (_sv.croquisTextos && _sv.croquisTextos.length) || (_sv.croquisSimbolos && _sv.croquisSimbolos.length) || (_sv.croquisLineas && _sv.croquisLineas.length));
  var _distData = JSON.stringify({ croquis: _sv.croquis || null, peines: _sv.croquisPeines || [], textos: _sv.croquisTextos || [], simbolos: _sv.croquisSimbolos || [], lineas: _sv.croquisLineas || [], legend: _P5_PIE_DISTRIB, legAspect: 2.776 });
  var _distCanvasHTML = '<canvas id="p5distcv" width="720" height="1280" style="width:100%;height:100%;display:block;border-radius:6px;background:#fff"></canvas>';
  for (var k = 0; k < 11; k++){
  ...
  return "<div class=\"sheet p5imgs\">"
    ...
    + "</style>"
    + "<div class=\"p5area\">" + "<div class=\"p5padbox\"><table class=\"p5padron\"><colgroup><col style=\"width:" + _padWdir + "%\"><col style=\"width:" + _padWuso + "%\"><col style=\"width:" + _padWsup + "%\"><col style=\"width:" + _padWbat + "%\"></colgroup><thead><tr><th>" + (_p5esc(cab) || "CATASTRO") + "</th><th>USO</th><th>SUP. m&#178;</th><th>BAT</th></tr></thead><tbody>" + filas + "</tbody></table></div>"
    + celdas
    + "</div>"
    + "</div>"
    + "";
...
var _P5V = { localesCon:4, localesSin:5, vivMasEntrada:6, entradasMas:7, comPunto1:8, comPunto2:9, contadorNum:12, ubicContador:11, llaveAcerado:10, matConexion:15, diamConexion:16, matMontante:20, cuartoUbic:21 };
...
function _p5memoria(R, meta, saved){
  ...
  var _vkv = {}; (saved.vkv||[]).forEach(function(p){ if(p&&p[0]!=null) _vkv[p[0]]=p[1]; });
  var _MIGV = {"__nid0":"ac_ncont","__nid1":"ac_ubic","__nid2":"ac_llave","__nid3":"mab_mat","__nid4":"cc_ubic","__nid5":"otros_t1_esp","__nid6":"otros_t2_esp","__nid7":"otros_eur_esp"};
  Object.keys(_MIGV).forEach(function(k){ if(_vkv[k]!=null && _vkv[_MIGV[k]]==null) _vkv[_MIGV[k]]=_vkv[k]; });
  var vgid = function(id){ return _vkv[id]!=null ? String(_vkv[id]) : ""; };
  ...
  var _peinesAct = (function(){ var grupos=[]; peines.forEach(function(pe){ if(!pe || !(pe.maIE||pe.maEng)) return; var ie = String(pe.maIE||"").toUpperCase().indexOf("EXT")>=0 ? "exterior" : "interior"; var eng = String(pe.maEng||"").toUpperCase().indexOf("BA\u00d1O")>=0 ? "ba\u00f1os" : "cocinas"; var pu = pe.puerta ? String(pe.puerta).trim() : ""; var g=null; for(var i=0;i<grupos.length;i++){ if(grupos[i].ie===ie && grupos[i].eng===eng){ g=grupos[i]; break; } } if(!g){ g={ie:ie,eng:eng,puertas:[]}; grupos.push(g); } if(pu) g.puertas.push(pu); }); return grupos.map(function(g){ var n=g.puertas.length; if(n===0){ return " El montante es "+g.ie+" y engancha en "+g.eng+"."; } if(n===1 && !/(^|\s)(Y|E)\s|[,\/+&]/i.test(g.puertas[0])){ return " El de las PUERTAS "+g.puertas[0]+" es "+g.ie+" y engancha en "+g.eng+"."; } var adj = (g.ie==="exterior") ? "exteriores" : "interiores"; return " Los de las PUERTAS "+_p5listaES(g.puertas)+" son "+adj+" y enganchan en "+g.eng+"."; }).join(""); })();
  ...
  var _ll = +m.llaves||0;
  // v18.164 -- Con 2 baterias y "SOLO PIECERIA" tambien salen 2 llaves, pero
  // sin la de fachada (una en cada bateria) -- distinto del caso normal de 1
  // bateria (fachada + bateria), que tambien da 2. Hay que distinguir el texto.
  var _esPzLl = String(m.longAli||"").trim()==="SOLO PIECERIA";
  var llaveTxt = _ll===1 ? "En la batería (1 ud)."
    : (_ll===2 ? (_esPzLl ? "Una en cada batería (2 uds)." : "Una en la fachada y otra en la batería (2 uds).")
    : (_ll===3 ? "Una en la fachada y una en cada batería (3 uds)." : ""));
  // Plantas de cada batería leídas de la columna BAT del catastro (editable). Locales y toma de comunidad van con la batería 1.
  var _catRows = Array.isArray(saved && saved.catastro) ? saved.catastro : [];
  function _p5plantaRankM(p){ var ss=String(p==null?"":p).trim().toLowerCase(); if(ss===""||ss==="0"||ss==="pb"||ss==="b"||ss.indexOf("baj")===0) return 0; if(ss.indexOf("atic")===0||ss.indexOf("átic")===0) return 900; var nn=parseInt(ss.replace(",","."),10); return isNaN(nn)?9000:nn; }
  function _p5plantaNomM(p){ var ss=String(p==null?"":p).trim(); if(ss==="") return ""; var nn=parseInt(ss.replace(",","."),10); if(isNaN(nn)){ var lo=ss.toLowerCase(); if(lo.indexOf("baj")===0||lo==="pb"||lo==="b") return "baja"; if(lo.indexOf("atic")===0||lo.indexOf("átic")===0) return "ático"; return ss; } if(nn===0) return "baja"; if(nn<0) return ""; return nn+"ª"; }
  function _p5plantasBatM(bn){ var seen={}, arr=[]; _catRows.forEach(function(f){ if(String((f&&f.bat)||"").trim()!==bn) return; var nom=_p5plantaNomM(f&&f.planta); if(!nom) return; if(!(nom in seen)){ seen[nom]=1; arr.push({k:_p5plantaRankM(f&&f.planta),t:nom}); } }); arr.sort(function(a,b){ return a.k-b.k; }); return arr.map(function(x){ return x.t; }); }
  var _bat1Pl=_p5plantasBatM("1"), _bat2Pl=_p5plantasBatM("2");
  var _hayBatCat=(_bat1Pl.length>0 || _bat2Pl.length>0);
  var _sufLocCom=(localesCon>0||localesSin>0)?(nCom>0?["locales","toma de comunidad"]:["locales"]):(nCom>0?["toma de comunidad"]:[]);
  var _batP1, _batP2;
  if(_hayBatCat){
    var _b1all=_bat1Pl.concat(_sufLocCom);
    _batP1 = _b1all.length ? (" (para las plantas "+_p5listaES(_b1all)+")") : "";
    _batP2 = _bat2Pl.length ? (" (para las plantas "+_p5listaES(_bat2Pl)+")") : "";
  } else {
    _batP1=" (para las plantas "+(pbaja?"baja, ":"")+"1ª, 2ª"+((localesCon===0&&localesSin===0&&nCom===0)?" y 3ª":", 3ª")+((localesCon>0||localesSin>0)?(nCom>0?", locales y toma de comunidad":" y locales"):(nCom>0?" y toma de comunidad":""))+")";
    var _batP2Arr=[]; for(var _bi=4;_bi<=plantas;_bi++){ _batP2Arr.push(_bi+"ª"); }
    _batP2=_batP2Arr.length?(" (para las plantas "+_p5listaES(_batP2Arr)+")"):"";
  }
  ...
  var armarioTxt = "Se colocar"+(bat2Tomas?"án":"á")+" "+_f154+", "+_p5esc(String(cuartoUbic||"").toLowerCase())+", con puertas de acceso "+(_puertaArm?("de "+_puertaArm):"")+"dotadas de rejillas de ventilación y cerradura normalizada por Emasesa.";
  ...
  <p class="meml">Está compuesta por planta baja${plantas?(" + "+plantas+" plantas"):""}${hayAtico?" + ático":""}, con un total de ${_p5listaES([nViv+" vivienda"+(nViv===1?"":"s"), localesTot>0?(localesTot+" local"+(localesTot===1?"":"es")):"", nCom>0?(nCom+" punto"+(nCom===1?"":"s")+" de comunidad ("+_p5esc(_p5listaES(comNoms.map(function(_cn){return "en "+_cn;})))+")"):""])}.${localesCon>0?(" "+localesCon+" local"+(localesCon===1?" tiene":"es tienen")+" suministro propio, por lo que dejaremos previsto su alojamiento."):""}</p>${fachReg?('<p class="meml">El Registro de Emasesa se encuentra a pie de calle (en la parte '+fachReg+' del edificio).'+(/-TRASLADAR/i.test(String(vgid("ac_llave")||""))?" El presente presupuesto únicamente será válido si, una vez aceptado, se obtiene el visto bueno de EMASESA para su traslado; si no, habrá de ser revisado.":"")+'</p>'):""}
  ...
  <p class="meml"><b>IDENTIFICACIÓN DE LAS VIVIENDAS:</b><br>${_p5identVivText(saved.catastro, pbaja, presto, patico)}</p>
  ...
  <p class="meml"><b>CONTADOR:</b><br>El contador${vgid("ac_ncont")?(" (nº "+_p5esc(vgid("ac_ncont"))+")"):""} está ubicado en ${_p5esc((vgid("ac_ubic")||"zonas comunes").toLowerCase().replace(/^en /,""))} del edificio.</p>
  ...
  <table class="memtab"><thead><tr><th>Batería nº</th><th>Nº de tomas</th><th>Nº de filas</th><th>Emplazamiento</th></tr></thead><tbody>
    <tr><td class="c">1</td><td class="c">${batTomas}</td><td class="c">${batFilas}</td><td class="c">${_f154}</td></tr>${bat2Tomas?('<tr><td class="c">2</td><td class="c">'+bat2Tomas+'</td><td class="c">'+bat2Filas+'</td><td class="c">'+_f154+'</td></tr>'):''}
  </tbody></table>
</div>`;
...
function renderPresupuesto(R, meta, dsg, cuadro, saved, docsGP){
  ...
  // Documento "Autorizacion paso de instalaciones": solo si la frase esta en Condiciones particulares.
  var _pasoDoc = null;
  if(docsGP && docsGP.pasoInstalaciones){
    var _cn = String(condiciones||"").normalize("NFD").replace(/[\u0300-\u036f]/g,"").toLowerCase();
    if(_cn.indexOf("paso de instalaciones a traves de su propiedad.") >= 0) _pasoDoc = docsGP.pasoInstalaciones;
  }
...
<style>
  ...
  *{ box-sizing:border-box; }
  ...
  .gpdoc .gpenc .legalp{ margin:0 0 3px; line-height:1.45; text-align:right; }
  .gpdoc .gpbody .legalp{ margin-bottom:34px; }                /* inter-parrafos del cuerpo: todos iguales */
  ...
  /* Hueco en blanco no utilizable: empuja el cuadro de totales/firmas hasta el pie.
     ALTURA AJUSTABLE AQUI, en un solo sitio. */
...
.fpsab{ position:absolute; right:5px; top:54%; transform:translateY(-50%); width:132px; height:auto; }
  ...
  @media print{
    ...
    .sheet.gpdoc{ min-height:230mm; }   /* documento grupo presion: llena la pagina para anclar el pie al fondo */
    .sheet.p5emasesa{ min-height:230mm; }   /* impreso EMASESA: ancla el pie legal al fondo en impresion */
...
<div class="docfoot"><div class="dfline"></div><div class="dfnp">Presupuesto Nº ${_p5esc(np)||""} <span style="font-size:8pt;color:#5b7fa6;">(${_p5esc(fecha)})(${_p5esc(rev)})</span></div></div>
...
function capCuarto(b39) {
  // Titulo FIJO: el tipo (aluminio/obra/existente) se muestra en la columna "material" del presupuesto.
  return "2.1 Armario batería";
}
function calcCuarto(nsum, b39, bat1, bat2, precios, obra) {
  ...
  // v18.164 -- Criterio Guille: con 2 baterias, armario ALUMINIO -> 2 unidades
  // (antes 1,5); armario de OBRA -> 2 puertas (antes 1), una por bateria.
  add("Armario aluminio con cerradura", "aluminio", (b39 === "ALUMINIO" ? (baterias >= 2 ? 2 : 1) : 0), "ALB", capB);
  add("Albañilería (tabique + enfoscado)", "ud", (esObra ? 1 : 0), "ALB", capB);
  add("Puerta", (b39 || ""), (esObra ? (baterias >= 2 ? 2 : 1) : 0), "ALB", capB);
...
function geomPeine(t, h, n, baja) {
  var P = parsePeine(t), alt = alturasPeine(P.base, P.a, P.b, P.c, n, P.d), tubo = 0, i;
  if (baja) {
    // El peine NACE ARRIBA (azotea) y BAJA: la longitud de cada tubo se mide desde el nivel
    // mas alto del edificio (ultima planta n + aticos c) hacia abajo -> el atico pasa a ser el
    // tubo corto y la planta baja el largo. La subida hasta la azotea va en los tramos (recorrido comun).
    var top = n + P.c + 2;
    for (i = 0; i < alt.length; i++) alt[i] = top - alt[i];
  }
  for (i = 0; i < alt.length; i++) tubo += alt[i];
  return { tubo: tubo * h, viv: alt.length, peine: alt.length ? Math.max.apply(null, alt) * h : 0 };
}
function pTuboS(t, h, n, baja) { return geomPeine(t, h, n, baja).tubo; }   // m de tubo VERTICAL del peine
function mPeineS(t, h, n, baja) { return geomPeine(t, h, n, baja).peine; } // m de peine VERTICAL (altura del peine)
...
// v18.162 -- Unico sitio que construye el objeto "entrada" a partir de lo
// guardado en el Sheet. Antes habia dos copias casi identicas (pantalla de
// Mediciones y PDF impreso) que habia que mantener sincronizadas a mano --
// asi fallo hoy con Malvaloca 1 (se arreglo una y se olvido la otra). Ahora
// las dos llaman a esta misma funcion: si falta un campo, falta en las dos
// pantallas a la vez, nunca solo en una.
function _p5Entrada(saved){
  var m = (saved && saved.motor) || {};
  var nViv = _contarViviendas(saved);
  var puntosCom = (saved.motor && +saved.motor.puntosComunidad) || 0;
  return {
    nsum: +m.nsum || 0,
    tipoSuministro: m.tipo,
    longTuboConexion: (m.longCon === "VALIDO" || m.longCon === "NO EXISTE") ? m.longCon : (+m.longCon || 0),
    viviendas: nViv,
    puntosComunidad: puntosCom,
    masDeUnaEntrada: +m.masDeUnaEntrada || 0,
    proyecto: false,
    grupoPresion: { seInstala: !!(+m.gpInstala || 0), tiene: false, modelo: "", deposito: "" },
    longAlimentacion: +m.longAli || 0,
    montajeAli: m.montaje || "",
    codosTermo: +m.codos || 0,
    llaves: +m.llaves || 0,
    bateria1: m.bat1 || "",
    bateria2: m.bat2 || "",
    tipoCuarto: m.tipoCuarto || "",
    otrosTiempos: +m.otrosTiempos || 0,
    otrosEur: +m.otrosEur || 0,
    gpMotAct: +m.gpMotAct || 0,
    gpPotAct: m.gpPotAct || "",
    gpNdepAct: +m.gpNdepAct || 0,
    gpTdepAct: m.gpTdepAct || "",
    gpInstala: m.gpInstala || "",
    gpPotNew: m.gpPotNew || "",
    gpModeloNew: m.gpModeloNew || "",
    gpNdepNew: +m.gpNdepNew || 0,
    gpTdepNew: m.gpTdepNew || "",
    gpDias: +m.gpDias || 0,
    gpLongExp: +m.gpLongExp || 0,
    peines: (saved && saved.peines) || [],
    zonas: (saved && saved.zonas) || {},
    plantas: +m.plantas || 0,
    altura: +m.altura || 0,
    peinesHDias: +m.peinesHDias || 0,
    pctBenefVenta: m.pctBenefVenta
  };
}
function paso2_peines(R, F) {
  ...
  var ag = { tuboTotal:0, tubo4y1:0, codos:0, codos4y1:0, viviendas:0, vintViv:0,
    ...
    // v18.164 -- Contador de PEINES (no viviendas) por tipo de enganche, para
    // el criterio nuevo de dias de EXT / INT-FACIL (por peine vertical).
    enganchePeines:{ "EXT":0, "INT-FACIL":0, "INT-MEDIO":0, "INT-DIFICIL":0 },
    diasVExt:[], avisos:[] };
  ...
  peines.forEach(function (pe, idx) {
    ...
    var eng = (pe.enganche || "").trim();
    if (ag.enganche[eng] != null) ag.enganche[eng] += M;
    // v18.165 -- Criterio Guille: EXT / INT-FACIL tambien por plantas -- hasta 4
...
function calcMontantes(R, precios, obra) {
  ...
  var dEXT   = Q.dEXT    != null ? Q.dEXT    : 1;
  var dFac   = Q.dFac    != null ? Q.dFac    : 1.5;
  ...
  // SIN tope: salen todos los peines, haya los que haya. El precio/día de cuadrilla es el mismo para todos
  // (446 €), así que se reutiliza el de la fila "-1" para los peines > 8 (que no tienen fila propia en el Sheet).
  var _prVext = precioDe(precios, "Fontanero (PEINE V-EXT -1)", "cuadrilla x2") || 0;
  for (var i = 1; i <= dv.length; i++) { var _cv = dv[i-1] || 0; L.push({ concepto: "Fontanero (PEINE V-EXT -" + i + ")", variante: "cuadrilla x2", cantidad: _cv, precio: _prVext, parcial: +(((_cv) * (_prVext)).toFixed(2)), tipoCoste: "MO", capitulo: MO }); }
  // aislamiento + chapa
  add("Aislamiento tubo PERT (coquilla)", "25mm", ag.canaletaTubo||0, "MAT", "4.1 Forrado de montantes");
  ...
  add("Chapa para canaleta (aluminio)", "2m", chapa, "MAT", "4.2 Canaleta de protección");
  add("Fontanero (doblado chapa canaleta)", "cuadrilla x2", chapa * fontCh, "MO", "4.2 Canaleta de protección");
  // enganches del montante actual por dificultad
  var enPe = ag.enganchePeines || {};
  // v18.164 -- porPeine: EXT e INT-FACIL cobran por PEINE VERTICAL (1 dia / 1,5
  // dias cada uno), no por vivienda. Los materiales (entronque, manguito, codo)
  // siguen yendo por vivienda en todos los casos, sin cambios.
  function bloqueEng(dif, conceptoDias, factor, conMedio, porPeine) {
    var q = en[dif] || 0;
  ...
  bloqueEng("INT-MEDIO", "Fontanero (ENGANCHE - interior medio)", dMed, true, false);
  bloqueEng("INT-DIFICIL", "Fontanero (ENGANCHE - interior difícil)", dDif, true, false);
  var total = 0; for (var j = 0; j < L.length; j++) total += L[j].parcial;
  return { lineas: L, total: +total.toFixed(2), avisos: (ag.avisos || []) };
}

// Modelo que se daba por defecto a cada potencia antes de poder elegir.
// Solo se usa para expedientes antiguos, que no tienen modelo guardado.
const MODELO_POR_DEFECTO = { "1,1":"CKE 2 Multi 35 4", "1,5":"CKE 2 Multi 35 5",
  "2,2":"CKE 2 Multi 35 6", "3":"CKE 2 Multi 35 8", "4":"CKE 2 Multi 35 10" };

// Texto visible del grupo (mediciones y punto 2 del presupuesto). Debe ser
// IGUAL al del desplegable de la toma de datos.
const ETIQUETA_GRUPO = {
  "CKE 2 Multi 35 4":  "1,1KW CKE 2 Multi 35 4",
  "CKE 2 Multi 35 5":  "1,5KW CKE 2 Multi 35 5",
  "AGUALECTRIC 2CV":   "1,5KW AGUALECTRIC (2cv)",
  "CKE 2 Multi 35 6":  "2,2KW CKE 2 Multi 35 6",
  "CKE 2 Multi 35 8":  "3KW CKE 2 Multi 35 8",
  "CKE 2 Multi 35 10": "4KW CKE 2 Multi 35 10",
};

// Calderin (litros) que lleva cada modelo de grupo. Se amplia AQUI cuando
// entre un modelo nuevo. Si un modelo no esta en la lista, se usa POR_DEFECTO.
const CALDERIN_POR_MODELO = {
  "AGUALECTRIC 2CV": "100",
};
const CALDERIN_POR_DEFECTO = "8";   // toda la gama CKE
function calderinDe(modelo){
  var m = String(modelo == null ? "" : modelo).trim();
  if (!m) return "";
  return CALDERIN_POR_MODELO[m] || CALDERIN_POR_DEFECTO;
}

function calcGrupoPresion(e, diamAli, precios) {
  e = e || {};
  var MOD = MODELO_POR_DEFECTO;
  ...
  // El modelo lo elige el usuario (desplegable). MOD queda de respaldo para
  // los expedientes antiguos, que no tienen modelo guardado.
  var modelo = (e.gpModeloNew == null ? "" : String(e.gpModeloNew).trim()) || MOD[pot] || "";
  var potConc = POT[pot] || "";
  var L = [];
  ...
  L.push({ concepto: "Grupo presión (by-pass + llaves + v.antiretorno + pequeño material)", variante: "ud",
           cantidad: hayGrupo, precio: (hayGrupo ? precioDe(precios, "Grupo presión (by-pass + llaves + v.antiretorno + pequeño material)", "ud") : 0),
...
function _finanTradicional(R){
  var viv=((R.entrada&&R.entrada.nsum)||0)-((R.entrada&&R.entrada.puntosComunidad)?1:0); // mismos comuneros que Plan 5 (viviendas+locales; la toma de comunidad no cuenta)
...
module.exports = function (app) {
  ...
  app.get(["/plan5", "/plan5/toma-datos"], async function (req, res) {
    ...
    try {
      ...
      if (dir) {
        const f = await leerFila(dir);
        if (f && f.row[6]) {
          // v18.163 -- Ademas del formulario (columna G), traemos los "puestos a
          // mano" de Mediciones (columna H, overrides de cantidad) para que el
          // aviso de Toma de Datos pueda decir si un peine ya tiene dias fijados
          // a mano, sin tener que ir a la otra pantalla a comprobarlo.
          try {
            const sObj = JSON.parse(f.row[6]);
            let ov = {};
            ...
            // la revisión con la que se cerró, no cambios posteriores.
            if (sObj.estado === "cerrado" && sObj.snapshot && sObj.snapshot.toma) {
              const _tc = sObj.snapshot.toma;
              if (_tc.v !== undefined) sObj.v = _tc.v;
              if (_tc.vkv !== undefined) sObj.vkv = _tc.vkv;
              if (_tc.zonas !== undefined) sObj.zonas = _tc.zonas;
              if (_tc.peines !== undefined) sObj.peines = _tc.peines;
              if (_tc.catastro !== undefined) sObj.catastro = _tc.catastro;
            }
            saved = JSON.stringify(sObj);
          } catch (eS) { saved = f.row[6]; }
        }
      }
    } catch (e) { /* sin Sheet/pestana aun: pantalla en blanco */ }
    try {
  ...
  // Reconstruye el CUADRO economico a partir de las lineas reales (p.ej. las de la FOTO ya con los
  // cambios manuales aplicados encima). Mismo camino que el motor: paso5 + paso6.
  function _p5CuadroDesdeLineas(saved, reales) {
    ...
    return {
      ...
      finCom: finanComunitaria(R.totales.conSubvencion).importe, finComPct: finanComunitaria(R.totales.conSubvencion).pct,
      subvTrad: _T.subvTrad, totSubvTrad: _T.totSubvTrad, comuneroTrad: _T.comuneroTrad, fin6Trad: _T.fin6Trad, fin12Trad: _T.fin12Trad, fin18Trad: _T.fin18Trad, finComTrad: _T.finComTrad, finComPctTrad: _T.finComPctTrad,
  ...
  async function p5DesgloseHandler(req, res) {
    ...
    try {
      ...
      if (sirviendoFoto) {
        ...
        if (dsg) dsg.foto = true;                    // la pantalla sabe que esto es una FOTO (no hay motor detras)
        if (estadoActual !== "cerrado") {
          // DESCONGELADO CON FOTO (modos "manual" y "solo precios"): la foto manda, pero los cambios
          // manuales de CANTIDAD y PRECIO de ESTA obra se aplican ENCIMA y se rehacen parciales,
          // totales de capitulo y cuadro economico. (Antes se servia la foto tal cual: se podia
          // escribir pero no cambiaba nada.)
          var _OV = (saved.overrides || {}), _OVP = (saved.overridesPrecio || {});
          var _accF = 0, _realesF = [], _lnsF = (dsg && dsg.lineas) || [];
          for (var _iF = 0; _iF < _lnsF.length; _iF++) {
            var _lF = _lnsF[_iF];
            if (_lF.tipo_fila === "capitulo") { _accF = 0; continue; }
            if (_lF.tipo_fila === "total") { _lF.parcial = Math.round(_accF * 100) / 100; continue; }
            var _kF = _lF.ovkey || (String(_lF.concepto || "") + "||" + String(_lF.variante || ""));
            _lF.ovkey = _kF; _lF.over = false; _lF.overP = false;
            if (Object.prototype.hasOwnProperty.call(_OVP, _kF)) { var _pF = _OVP[_kF]; if (_pF !== "" && _pF != null && !isNaN(+_pF)) { _lF.precio = +_pF; _lF.overP = true; } }
            if (Object.prototype.hasOwnProperty.call(_OV, _kF)) { var _vF = _OV[_kF]; if (_vF !== "" && _vF != null && !isNaN(+_vF)) { _lF.cantidad = +_vF; _lF.over = true; } }
            _lF.parcial = Math.round(((+_lF.cantidad || 0) * (+_lF.precio || 0)) * 100) / 100;
            _accF += _lF.parcial; _realesF.push(_lF);
          }
          try { cuadro = _p5CuadroDesdeLineas(saved, _realesF); } catch (eCF) { console.error("[plan5] cuadro sobre foto:", eCF.message); }
        }
      }
      ...
      if (!sirviendoFoto && m && m.nsum && m.tipo && (m.longCon != null && m.longCon !== "")) {
        ...
        var nViv = _contarViviendas(saved);
        var puntosCom = (saved.motor && +saved.motor.puntosComunidad) || 0;
        var R = calcular({ entrada: _p5Entrada(saved) },
        ...
        var pintarCapMont = function (titulo, calc) {
          ...
          calc.lineas.forEach(function (l) {
            ...
            // v18.166 -- Criterio Guille: peines verticales y enganche EXT/INT-FACIL,
            // por plantas. Mismo formato visual que 'tubo alimentación' (tramos con
            // flechas), pero con plantas en vez de metros y el ultimo tramo abierto
            // ("X plantas o más"), editable, con override por obra igual que siempre.
            if (l.concepto === "Fontanero (ENGANCHE - exterior)") {
              var _cE = om.engExtCorte != null ? om.engExtCorte : 4;
      ...
      // Los NOMBRES de partida se leen SIEMPRE de plan5_mediciones (Sheet), tambien al servir una foto:
      // la foto congela importes, pero el rotulo lo manda el Sheet (cambiarlo alli se refleja en pantalla).
      try {
        var _metaN = (med && med.meta) ? med.meta : ((await leerMediciones((saved && saved.overridesDato) || {})).meta || {});
        if (dsg && dsg.lineas && _metaN) {
          var _mapN = {};
          for (var _k in _metaN) { var _cp = _metaN[_k] && _metaN[_k].capitulo_presupuesto; if (_cp) { var _n = _p5parseCap(_cp).num; if (_n && !(_n in _mapN)) _mapN[_n] = _cp; } }
          dsg.lineas.forEach(function (l) { if (l && l.capitulo_presupuesto) { var _ln = _p5parseCap(l.capitulo_presupuesto).num; if (_ln && _mapN[_ln]) l.capitulo_presupuesto = _mapN[_ln]; } });
        }
      } catch (eN) { console.error("[plan5] refresco nombres:", eN.message); }
  ...
  // Genera variantes del NOMBRE de la via para que el Catastro la reconozca aunque el
  // usuario use su propia nomenclatura ("Av de la Calesera", "de la Calesera"...). El
  // Catastro espera el nombre LIMPIO ("CALESERA"), sin el tipo de via ni el articulo.
  // Se prueban de mas limpia a mas literal; la primera que el Catastro reconozca, gana.
  function _p5catNombreVariantes(nombre){
  ...
  async function _p5catResolverVia(base, prov, muni, sig, nombre){
    ...
    for (var k = 0; k < variantes.length; k++){
      var nom = variantes[k];
      var vias = await _p5catConsultaVia(base, prov, muni, sig, nom);
      if(!vias.length && sig) vias = await _p5catConsultaVia(base, prov, muni, "", nom);
      if(!vias.length) continue;
      var exact = vias.filter(function(v){ return v.nv === nom; });
  ...
  // ===== Consulta por REFERENCIA CATASTRAL (fallback cuando la calle no sale en la lista) =====
  app.get("/plan5/catastro/porref", async function (req, res) {
    if (!validToken((req.query && req.query.token) || "")) return res.status(403).json({ ok: false, error: "token" });
    try {
      var q = req.query || {};
      var prov = _p5catProvincia(q.cp || "");
      var muni = _p5catNorm(q.poblacion || "");
      var rc = String(q.rc || "").toUpperCase().replace(/[^0-9A-Z]/g, "").slice(0, 14);
      if (!prov || !muni) return res.json({ ok: false, error: "Falta la poblacion o el codigo postal del expediente." });
      if (rc.length < 14) return res.json({ ok: false, error: "Referencia catastral no valida: deben ser al menos 14 letras y numeros." });
      var base = "https://ovc.catastro.meh.es/ovcservweb/OVCSWLocalizacionRC/OVCCallejero.asmx";
      var listUrl = base + "/Consulta_DNPRC?Provincia=" + encodeURIComponent(prov) + "&Municipio=" + encodeURIComponent(muni) + "&RC=" + encodeURIComponent(rc);
      var listXml = await _p5catFetch(listUrl);
      var ferr = _p5catError(listXml);
      if (ferr) return res.json({ ok: false, error: "Catastro: " + ferr });
      var items = _p5catListaInmuebles(listXml);
      if (!items.length) return res.json({ ok: false, error: "El Catastro no devuelve inmuebles para la referencia " + rc + ". Revisa que sea correcta." });
      if (items.length > 75) items = items.slice(0, 75);
      var plano = null;
      try { if (rc.length === 14) { plano = await _p5catCoordParcela(prov, muni, rc); if (plano) { var _ed = await _p5catCoordEdificios(rc); plano.edificios = (_ed && _ed.edificios) || []; plano.patios = (_ed && _ed.patios) || []; } } } catch (e3) {}
      var filas = [];
      for (var i = 0; i < items.length; i++) {
        var it = items[i];
        var det = { uso: "", sup: "", planta: "", puerta: "" };
      ...
      console.error("[plan5] catastro porref error:", e.message);
      res.status(500).json({ ok: false, error: e.message });
    }
  });
  ...
  app.post("/plan5/cerrar", async function (req, res) {
    ...
    try {
      ...
      // v(hoy) — Antes de congelar, guardamos también una COPIA de los datos en
      // bruto de Toma de Datos (catastro, viviendas, peines y el resto del
      // formulario) tal y como estaban en este momento. Sin esto, el desglose y
      // el cuadro quedaban fijos con la revisión de cierre, pero los avisos de
      // "Tipo de peines" seguían leyendo los campos vivos — si alguien los
      // tocaba más tarde (o cambiaban con una reimportación de Catastro), la
      // pantalla mostraba avisos de HOY encima de un presupuesto ya cerrado.
      var _tomaCongelada = {
        v: saved.v, vkv: saved.vkv, zonas: saved.zonas, peines: saved.peines,
        catastro: saved.catastro,
      };
  ...
  app.get(["/plan5/presupuesto", "/plan5/presupuesto/:nombre"], async function (req, res) {
    ...
    try {
      ...
      var R = { finca: ficha, meta: meta, entrada: _p5Entrada(savedExp) };
      ...
      try {
        ...
        var _paso=_pick("paso_instalaciones");
        _docsGP = { mantener:(_man && _man.activo!=="0")?_man:null, renunciar:(_ren && _ren.activo!=="0")?_ren:null, encabezado:(_enc && _enc.activo!=="0")?_enc.cuerpo:"", pie:(_pie && _pie.activo!=="0")?_pie.cuerpo:"", pasoInstalaciones:(_paso && _paso.activo!=="0")?_paso:null };
  ...
  // Catastro guardado en Toma de Datos (columna G, datos_json.catastro): filas {planta, puerta, uso, sup, bat}.
  // El catastro no forma parte de la foto congelada, asi que vale igual con el presupuesto abierto o cerrado.
  // Lo usa documentacion.cjs (inicializarEstadosFase) para dar de alta los pisos al entrar en fase 05.
  async function _p5CatastroDe(direccion) {
    var f = await leerFila(direccion);
    if (!f || !f.row || !f.row[6]) return null;
    var g = null; try { g = JSON.parse(f.row[6]); } catch (e) { return null; }
    return (g && Array.isArray(g.catastro)) ? g.catastro : null;
  }
  app.locals.plan5 = Object.assign(app.locals.plan5 || {}, { leerCatastro: _p5CatastroDe });
};
