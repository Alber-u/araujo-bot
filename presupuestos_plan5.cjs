...
function paso4_desglose(R, F) {
  ...
  pushCap(R.alimentacion); pushCap(R.cuarto); pushCap(R.grupo); pushCap(R.otros);
}
    ...
    try {
      ...
      if (m && m.nsum && m.tipo && (m.longCon != null && m.longCon !== "")) {
        ...
        var R = calcular({ entrada: { nsum: +m.nsum || 0, tipoSuministro: m.tipo, longTuboConexion: +m.longCon || 0,
                           ...
                           otrosTiempos: +m.otrosTiempos || 0, otrosEur: +m.otrosEur || 0, gpMotAct: +m.gpMotAct || 0, gpInstala: m.gpInstala || "", gpPotNew: m.gpPotNew || "", gpNdepNew: +m.gpNdepNew || 0, gpTdepNew: m.gpTdepNew || "", gpDias: +m.gpDias || 0, gpLongExp: +m.gpLongExp || 0 } },
