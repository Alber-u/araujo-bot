...
const _P5_REVISION = "Rev-19.16 24/06/2026";
...
function _p5paginaImagenes(R, meta, cuadro, saved){
  ...
  var ETIQ = ["LLAVE DE ACERADO","PORTERO ELECTRONICO","CONTADOR","CATASTRO","ARMARIO CONTADOR","NUEVO ARMARIO","PEINE 1","PEINE 2","DISTRIBUCION","IMAGEN 10","IMAGEN 11","IMAGEN 12"];
  ...
  return "<div class=\"sheet p5imgs\">"
    + "<style>"
    + ".p5imgs .p5wrap{display:flex;gap:12px;align-items:flex-start;margin:6px 0 12px}" + ".p5imgs .p5padron{width:230px;flex:0 0 230px;border-collapse:collapse;font-size:9px;table-layout:fixed}"
    ...
    + ".p5imgs .p5grid{flex:1;display:grid;grid-template-columns:repeat(3,1fr);gap:6px}"
    ...
    + "</style>"
    + "<div class=\"p5wrap\">" + "<table class=\"p5padron\"><colgroup><col style=\"width:48%\"><col style=\"width:26%\"><col style=\"width:26%\"></colgroup><thead><tr><th>" + (_p5esc(cab) || "CATASTRO") + "</th><th>USO</th><th>SUP. m&#178;</th></tr></thead><tbody>" + filas + "</tbody></table>"
...
<script>try{window.__P5_TIT__=${JSON.stringify(_p5fname)};document.title=window.__P5_TIT__;}catch(e){}</script>
<style>
  :root{ --navy:#004079; }
...
<div class="p5toolbar">
  ...
  <button class="p5btn" onclick="p5imprimir()">&#128424; Imprimir / Guardar PDF</button><script>function p5imprimir(){var b=document.querySelector('.p5btn');if(b&&b.getAttribute('data-busy')){return;}var L=[].slice.call(document.querySelectorAll('.p5img'));var pend=L.filter(function(im){return im.getAttribute('data-src');});if(!pend.length){try{if(window.__P5_TIT__)document.title=window.__P5_TIT__;}catch(e){}window.print();return;}if(b){b.setAttribute('data-busy','1');b.disabled=true;}var total=pend.length,hechas=0;function upd(){if(b)b.textContent='Cargando fotos... '+hechas+'/'+total;}upd();function fin(){if(b){b.disabled=false;b.innerHTML='&#128424; Imprimir / Guardar PDF';b.removeAttribute('data-busy');}requestAnimationFrame(function(){requestAnimationFrame(function(){setTimeout(function(){try{if(window.__P5_TIT__)document.title=window.__P5_TIT__;}catch(e){}window.print();},300);});});}function done(){hechas++;upd();if(hechas>=total){fin();}}pend.forEach(function(im){var url=im.getAttribute('data-src');fetch(url).then(function(r){if(!r.ok)throw 0;return r.blob();}).then(function(bl){return new Promise(function(res,rej){var fr=new FileReader();fr.onload=function(){res(fr.result);};fr.onerror=rej;fr.readAsDataURL(bl);});}).then(function(durl){im.onload=function(){im.style.display='';var ph=im.parentNode.querySelector('.p5ph');if(ph)ph.style.display='none';im.removeAttribute('data-src');done();};im.onerror=function(){im.removeAttribute('data-src');done();};im.src=durl;}).catch(function(){im.removeAttribute('data-src');done();});});}</script>
</div>
