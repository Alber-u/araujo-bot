...
// Build: 2026-06-14 v0.3 (Sobre v0.2: cierra el hueco para VER el tubo de conexión en MEDICIONES. (1) TOMA DE DATOS ahora CUENTA viviendas: el "Nº de viviendas" de cada fila pasa de texto derivado a INPUT editable (clase .vn) que suma en nsum -> el motor ya recibe nsum, tipo y longitud (objeto motor:{nsum,tipo,longCon} en el payload). (2) MOTOR endurecido: precioDe casa la variante por texto exacto Y, si es un número puro (diámetro), también numéricamente -> encuentra el "75"/"90"/"110" aunque el Sheet lo guarde como número (antes esas 3 líneas habrían salido a 0 €). Revalidado: Sánchez Pizjuán sigue dando 9 líneas + 1.293 €, con tipo como texto y como número. node --check OK. (Para ver número en vivo: re-guardar el presupuesto con esta versión y teclear el nº de viviendas por fila.))
