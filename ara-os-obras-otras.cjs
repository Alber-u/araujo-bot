...
function registrar(app) {
  ...
  // ============================================================
  // v0.2.0 — POST /obras-otras/:id/vincular-factura
  // Vincula una factura existente de Holded a esta OO.
  // Body: { invoice_id, invoice_num? }
  //   - Guarda holded_invoice_id en la sheet.
  //   - NO cambia la fase automáticamente.
  // ============================================================
  ...
  app.post("/api/ara-os/obras-otras/:id/vincular-factura", jsonBodyParser, async (req, res) => {
    ...
    try {
      ...
      const antes = obra.holded_invoice_id || "";
      if (antes === invoice_id) {
        return res.json({ ok: true, obra, sin_cambios: true });
      }
      const cambios = { holded_invoice_id: { antes, despues: invoice_id } };
      obra.holded_invoice_id = invoice_id;
      obra.updated_at = nowIso();
      obra.updated_by = body.usuario || "ARA OS";
      ...
      tryHistorial("factura_vinculada", obra, cambios, obra.updated_by);
  ...
  // ============================================================
  // v0.2.0 — DELETE /obras-otras/:id/factura-vinculada
  ...
  app.delete("/api/ara-os/obras-otras/:id/factura-vinculada", jsonBodyParser, async (req, res) => {
    ...
    try {
      ...
      const antes = obra.holded_invoice_id || "";
      if (!antes) {
        return res.json({ ok: true, obra, mensaje: "No había factura vinculada" });
      }
      obra.holded_invoice_id = "";
      obra.updated_at = nowIso();
      obra.updated_by = (req.body && req.body.usuario) || "ARA OS";
      ...
      tryHistorial("factura_desvinculada", obra,
        { holded_invoice_id: { antes, despues: "" } }, obra.updated_by);

      res.json({ ok: true, obra, antes });
    } catch (e) {
  ...
  console.log("[ara-os-obras-otras v0.2.0] Módulo cargado. 10 endpoints: ping + CRUD + tipos + fases + vincular/desvincular factura");
}
