...
function registrar(app) {
  ...
  // ¡Antes de /obras-otras/:id para que no lo capture como id!
  ...
  app.get("/api/ara-os/obras-otras/rentabilidad-todas", async (req, res) => {
    ...
    try {
      ...
      let obras = await leerObras();
      // Órdenes reales (excluye presupuestos). leerObras ya excluye borradas.
      obras = obras.filter(o => o && o.obra_id && o.fase !== "PRESUPUESTO");
